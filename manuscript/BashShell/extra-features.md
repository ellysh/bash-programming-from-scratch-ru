...
Такое перенаправление работает только в Bash, но не в Bourne shell (sh). Если вам важна POSIX-совместимость, перенаправляйте поток ошибок в поток вывода с помощью команды `2>&1`:
{line-numbers: false, format: Bash}
```

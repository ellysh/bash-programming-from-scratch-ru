...
Утилита info выводит свой результат на поток вывода. Далее в команде идёт символ `|`. Он означает, что вывод команды слева передаётся на ввод команде справа. Таким образом утилита grep получит текст справки по Bash. В этом тексте мы ищем строки со словом "GNU". Если Bash распространяется под [GNU GPL](https://ru.wikipedia.org/wiki/GNU_General_Public_License) лицензией, вывод grep будет непустым.

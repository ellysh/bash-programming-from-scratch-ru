...
1. Нет ограничения на количество строк передаваемых строк.
2. Нет ручных операций с буфером обмена.

Bash позволяет перенаправлять ввод и вывод команд в файлы. Другими словами ваше приложение должно уметь читать текстовые данные на входе и выводить результат на экран. Всю остальную работу по перенаправлению этих данных берёт на себя Bash.

Рассмотрим пример. Предположим, что мы ищем утилитой find файлы на диске. Результаты поиска надо сохранить в новый файл. Чтобы решить эту задачу, воспользуемся оператором перенаправления `1>`. Тогда команда вызова утилиты будет выглядить так:
{line-numbers: false, format: Bash}
```
find / -path */doc/* -name README 1> readme_list.txt
```

После выполнения этой команды в текущем каталоге будет создан файл `readme_list.txt`. В него запишется вывод утилиты find. Формат вывода такой же, как если бы он печатался на экран. Если файл с именем `readme_list.txt` уже существует в текущем каталоге, его содержимое будет перезаписано.

Что означает оператор `1>`? Это [перенаправление стандартного потока вывода](https://ru.wikipedia.org/wiki/Перенаправление_ввода-вывода). В Unix-окружении есть три [стандартных потока](https://ru.wikipedia.org/wiki/Стандартные_потоки). Они приведены в таблице 2-12.

{caption: "Таблица 2-12. Стандартные потоки POSIX", width: "100%"}
| Номер | Название | Применение |
| --- | --- | --- |
| 0 | Стандартный поток ввода (standard input или stdin). | Данные, которые передаются на вход программы. Они поступают с устройства ввода типа клавиатуры. |
|  | | |
...
|  | | |
| 2 | Стандартный поток ошибок (standard error или stderr). | Данные об ошибках, которые выводит программа. Как правило, они печатаются так же в окне терминала. |

Приложение работает в програмном окружении операционной системы. Поток представляет собой канал связи между ними. В ранних Unix-системах информация вводилась и выводилась через физические каналы. Ввод был привязан к клавиатуре, а вывод к монитору. Потоки ввели как [абстракцию](https://ru.wikipedia.org/wiki/Уровень_абстракции_(программирование)) над этими каналами. Абстракция позволила работать с различными объектами по одному и тому же алгоритму. Так ввод с реального устройства можно заменить на ввод из файла. Аналогично печать на экран можно заменить на вывод в файл.

Назначение потоков ввода и вывода понятно. Но зачем нужен поток ошибок? Он нужен для удобства. Представьте, что вы запустили утилиту find для поиска файлов. К некоторым каталогам у вас нет доступа. При попытке прочитать их содержимое выводится сообщение об ошибке. Если утилита нашла много файлов, сообщения об ошибках потеряются в её выводе. Чтобы прочитать только сообщения об ошибках, просто разделите поток вывода и ошибок. Один можно перенаправить в файл, а другой печатать на экран. Тогда все сообщения утилиты станут очевидны.

Чтобы перенаправить стандартный поток ошибок, воспользуйтесь оператором перенаправления `2>`. Например, вызов утилиты find будет выглядить так:
{line-numbers: false, format: Bash}
```
...
В этой команде используется интерфейс утилиты grep для обработки стандартного потока ввода. Но grep умеет сама читать содержимое указанного файла. Поэтому лучше в неё всегда передавать имя файла. Например так:
{line-numbers: false, format: Bash}
```
grep "Bash" README.txt
```

Рассмотрим пример посложнее. Некоторые руководства Bash рекомендуют вывод содержимого файла на экран с помощью встроенной команды echo. Её вызов выглядит так:
{line-numbers: false, format: Bash}
```
echo $( 0< README.txt )
```
...
Механизм подстановки результата выполнения команды в место её вызова называется **подстановкой команд**. Когда интерпретатор встречает символы `$(` и `)`, он выполняет заключённую между ними команду и подставляет её вывод.

В результате подстановки команд наш вызов echo выполняется в два этапа:

1. Передача содержимого файла `README.txt` на стандартный поток ввода.

2. Вывод данных из стандартного потока ввода командой echo.

При подстановке команд всегда учитывайте порядок их исполнения. Порядок исполнения соответствует порядку следования команд. Только закончив все подстановки, интерпретатор выполнит получившуюся команду целиком.

Из-за нарушения порядка исполнения следующий вызов find завершится с ошибками:
...
Эта команда должна была вывести содержимое всех файлов, найденных утилитой find. Но вместо этого мы получили ошибку от Bash.
...
Чтобы вызов find сработал, используйте утилиту cat для вывода содержимого файлов. Например так:
{line-numbers: false, format: Bash}
```
find / -path */doc/* -name README -exec cat {} \;
```

Операторы перенаправления стандартных потоков вывода и ввода используются часто. Поэтому для них ввели краткую форму: `>` и `<` соответственно. Перепишем наши примеры с вызовами find и echo с краткой формой перенаправления:
{line-numbers: true, format: Bash}
```
...
Рассмотрим ещё один пример использования конвейеров. GNU утилита `du` позволяет оценить используемое дисковое пространство. Если запустить её без параметров в текущем каталоге, она **рекурсивно** обойдёт все подкаталоги и выведет число байт, занимаемым на диске каждым из них.

I> В данном случае под рекурсивным обходом подразумевается повторение операции перехода в подкаталог. То есть утилита находит каталог. Затем она переходит в него и проверяет его содержание. Если обнаруживается подкаталог, утилита переходит в него и опять проверяет содержание. Это продолжается до тех пор, пока не будет достигнут каталог без подкаталогов. В этом случае утилита возвращается на один уровень выше и проверяет оставшиеся в нём подкаталоги. Алгоритм заканчивается, когда всё дерево каталогов оказывается пройденным.

Утилита принимает на вход список путей до каталогов, которые необходимо рекурсивно обойти. Также ей можно передать список путей до файлов. Тогда она оценит дисковое пространство, занимаемое только ими.
...
Есть ряд GNU утилит, которые получают входные данные из стандартного потока ввода. Обычно они применяются вместе с конвейерами для поиска и анализа текста. Таблица 2-13 приводит наиболее часто используемые из них.
...
{caption: "Иллюстрация 2-27. Результат обработки вывода утилиты `ls` с помощью `grep`", width: "100%"}
...
Иллюстрация 2-28 демонстрирует результаты выполнения в обоих случаях.

{caption: "Иллюстрация 2-28. Результаты выполнения команд с конвейером и разделителем", width: "100%"}

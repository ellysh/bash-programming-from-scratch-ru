...
Как вы возможно уже догадались, цифра перед знаком больше означает номер потока. Для перенаправления стандартного потока ввода, используется оператор `0<`:
{line-numbers: false, format: Bash}
```
...
Рассмотрим ещё один пример. Конвейеры часто используются для анализа вывода команд `ls`. Если вам надо получить имена файлов, в которых встречается некоторое слово, вы можете воспользоваться командой `find`. Но вместо этого можно сочетать команды `ls` и `grep`. Результат получится короче и удобнее для чтения:
{line-numbers: false, format: Bash}
```
ls /usr/share/doc/bash | grep html
```

W> Применяйте конвейеры для обработки результата `ls` только для вывода данных на экран в режиме командного интерпретатора. Не делайте этого в скриптах и командах манипуляции файлами!
...
Если вы запустите эту команду, то увидите вывод `ls` на экране. При этом он же будет записан в файл `result.txt`. Опция `-a` утилиты `tee` позволит вам дописывать в конец уже существующего файла, а не очищать его.
...
В результате выполнения этой команды, вывод `ls` будет сохранён в файле `ls.txt`, а `grep` — в `grep.txt`. Конечный результат по-прежнему окажется в `result.txt`. Такой раздельный вывод может помочь в поиске ошибок.

Мы уже знаем, как с помощью параметра `-exec` команды `find` выполнить действие над каждым найденным файлом. Возможно ли добиться подобного поведения с помощью утилиты `ls` и конвейеров? Да, это возможно. Но избегайте этого подхода, если вы работаете не в интерактивном режиме интерпретатора, а пишете скрипт. В общем случае лучше всегда использовать `find`. Здесь мы рассамотрим пример с конвейрами исключительно для образовательных целей.
...
Если в нашем распоряжении только встроенные средства интерпретатора Bash, ситуация меняется. Предположим, что мы запускаем утилиту `ls` и фильтруем её вывод с помощью `grep`. Bash не имеет доступа к промежуточным результатам работы каждой из этих утилит. Он получает конечный результат целиком в стандартном потоке вывода. Если вы хотите скопировать файлы, имена которых отфильтровал `grep`, просто конвейер не подойдёт:
{line-numbers: false, format: Bash}
```
ls /usr/share/doc/bash/* | grep html | cp -t ~
```

I> Опция `-t` утилиты `cp` позволяет изменить порядок следования копируемых файлов и целевого каталога. Этот каталог указыватся как первый параметр (в нашем случае `~`), а список файлов идёт следом за ним. 

Такой вызов утилиты `cp` после конвейера завершится ошибкой. Причина в том, что она не читает стандартный поток ввода, а ожидает имя копируемого файла во втором и последующих параметрах. Чтобы решить эту проблему, нам необходимо согласовать данные, полученные из конвейера, с форматом параметров утилиты `cp`. Другими словами, наша задача заключается в том, чтобы сконструировать строку параметров из данных, полученных на стандартном потоке ввода. Утилита `xargs` делает именно то, что нам нужно. Добавим её вызов в нашу команду следующим образом:
{line-numbers: false, format: Bash}
```
ls /usr/share/doc/bash/* | grep html | xargs cp -t ~
```

I> Обратите внимание на маску `/usr/share/doc/bash/*`, переданную на вход `ls`. Благодаря ей, утилита выведет абсолютные пути для каждого файла. Они нужны команде `cp`. Альтернативным решением может быть предварительный переход в каталог `/usr/share/doc/bash`. Тогда команду копирования можно упростить так:
{line-numbers: true, format: Bash}
```
...
В этом вызове стандартный поток вывода команды `grep` передаётся на вход `xargs`. Далее `xargs` вызывает утилиту `cp` с параметром `-t ~`. После него `xargs` подставляет каждую строку из вывода `grep` в качестве параметра командной строки. Обратите внимание, что файлы в выводе `grep` разделены символом перевода строки, а `xargs` разделяет полученные параметры пробелами.

Рассмотрим пример. Предположим, что вывод утилиты `grep` состоит из двух HTML файлов `bash.html` и `bashref.html`. Тогда `xargs` подставит их в конец вызова `cp`. После подстановки он будет выглядеть следующим образом:
{line-numbers: false, format: Bash}
```
cp -t /home/ilya.shpigor /usr/share/doc/bash/bash.html /usr/share/doc/bash/bashref.html
```

Такая подстановка происходит в памяти утилиты `xargs` и по умолчанию не выводится на экран. Вы можете включить функцию вывода сформированной команды с помощью опции `-t`. Например:
{line-numbers: false, format: Bash}
```
ls /usr/share/doc/bash/* | grep html | xargs -t cp -t ~
```

I> Способность представлять в уме то, что будет делать написанная вами программа — это важнейший навык программиста. Конечно, всегда можно включить отладочный вывод и проверить. Однако, умение прокручивать исполнение кода в уме значительно ускоряет проектирование, разработку и поиск ошибок. Тренируйте этот навык.

Может возникнуть ситуация, когда стандартный поток ввода содержит слишком много данных. В то же время у утилит типа `rm` и `cp` есть ограничение на число входных параметров. В таких случаях `xargs` позволяет разделить данные, полученные из потока ввода, на части и вызывать указанную утилиту для каждой из них. Для такого разделения используется опция `-l` с указанием числа строк. Например так:
{line-numbers: false, format: Bash}
```
ls /usr/share/doc/bash/* | grep html | xargs -l2 cp -t ~
```

I> Обратите внимание, что все параметры утилиты `xargs`, указываемые как опции, пишутся без пробела. Например, в команде выше мы пишем `-l2`, а не `-l 2`. Большинство GNU утилит поддерживают оба варианта записи, но не `xargs`.

В результате для каждых двух файлов из вывода `ls` будет происходить отдельный вызов утилиты `cp`.

По умолчанию утилита `xargs` добавляет параметры, сконвертированные из стандартного потока ввода, в конец указанной команды. Могут быть ситуации, когда требуется указать место их подстановки. В этом случае можно воспользоваться параметром `-I`. Например:
{line-numbers: false, format: Bash}
```
ls /usr/share/doc/bash/* | grep html | xargs -I% cp % ~
```

Параметр `-I%` сообщает утилите `xargs`, что подстановку надо совершить в место символа `%` в следующем далее вызове. При этом утилита `cp` будет вызвана для каждой строки, переданной на ввод утилите `xargs`. В нашем случае сформируются следующие команды:
{line-numbers: true, format: Bash}
```
...
|  | | |
| `grep` | Поиск по указанному шаблону. | `cat file.txt | grep GNU` |
| | | `info cat | grep -A 3 -B 3 GNU` |
|  | | |
| `tee` | Перенаправление потока ввода одновременно в поток вывода и в файл. | `cat file.txt | grep GNU | tee result.txt` |
|  | | |
| `sort` | Сортировка строк в потоке ввода в прямом и | `cat file.txt | sort -r` |
| | обратном (параметр `-r`) порядке. | `cat file.txt | sort -r` |
|  | | |
| `wc` | Подсчёт строк (параметр `-l`), слов (`-w`), букв | `cat file.txt | wc -l` |
| | (`-m`) и байтов (`-с`) в указанном файле или потоке ввода. | `info cat | wc -m` |
|  | | |
| `head` | Вывести указанное число байтов (параметр | `info cat | head -n 10` |
| | `-c`) или строк (`-n`) файла или текста из начала потока ввода. | `info cat | head -10` |
|  | | |
| `tail` | Вывести указанное число байтов (параметр | `info cat | tail -n 10` |
| | `-c`) или строк (`-n`) файла или текста из конца потока ввода. | `info cat | tail -10` |
|  | | |
...
В данном конкретном случае команда работает корректно потому, что все файлы содержат в своих именах только символы латинского алфавита и точки. Но предположим, что вы решили воспользоваться этой же командой для домашнего каталога:
{line-numbers: false, format: Bash}
```
ls ~ | grep html
```

В этом случае нет никакой гарантии, что команда отработает корректно. Почему? Обратимся к конкретному примеру.
...
I> Основное назначение утилиты `touch` — изменять время последней модификации файла на текущее. При этом если указанного файла не существует, он будет создан пустым.

Также для примера создайте файлы `test1.txt` и `file1.txt`:
{line-numbers: false, format: Bash}
```
...
Обратите внимание, что имя файла `test\nfile.txt` оказалось обрезано. Команда `ls` сама по себе выводит его в полной форме `'test'$'\n''file.txt'`. Однако, при передаче через конвейер символ `\n` заменяется на перевод строки и имя файла оказывается разделено. В результате этого `grep` обрабатывает его части по отдельности. В часть `test` соответствует шаблону поиска и выводится на экран, а часть `file.txt` — нет.

Это не единственная проблема обработки вывода утилиты `ls`. Предположим, что вы копируете файлы, один из которых содержит пробел в имени (например, `test file.txt`). Это приведёт к ошибке выполнения следующей команды:
{line-numbers: false, format: Bash}
```
ls ~ | xargs cp -t ~/tmp
```
...
Ошибка в том, что ни одного из файлов `test` и `file.txt` не существует. Из-за механизма word splitting вместо имени одного файла, состоящего из двух слов, мы получили два имени. Эту проблему можно решить с помощью двойных кавычек следующим образом:
{line-numbers: false, format: Bash}
```
ls ~ | xargs -I% cp -t ~/tmp "%"
```

Но если в имени файла стоит перевод строки, а не пробел, добавление кавычек не поможет. Как быть в такой ситуации? Ответ прост — не используйте утилиту `ls`. С этой задачей прекрасно справится `find`, вызванная с действием `-exec`. Например так:
{line-numbers: false, format: Bash}
```
...
Из рассмотренных нами примеров можно вывести две рекомендации:

1. Используйте конвейеры для обработки имён файлов с осторожностью.

2. Избегайте обработки вывода утилиты `ls`. Вместо неё используйте `find` с действием `-exec`. Если без `xargs` не обойтись, пользуйтесь опциями `-0` и `-print0`.
...
Стандарт POSIX, которому следует Bash, требует, чтобы каждая запущенная программа всегда завершалась с определённым [**кодом возврата**](https://ru.wikipedia.org/wiki/Код_ответа). При её успешном выполнении, этот код равен нулю. В противном случае он может принимать любое значение от 1 до 255. Когда вы применяете логический оператор к команде, на самом деле он вычисляет её код возврата. А чтобы его вычислить, команду необходимо сначала исполнить.
...
Кроме рассмотренных нами логических операторов команды в языке Bash могут разделяться точкой с запятой `;`. Он используется в случае, когда вызовы должны выполнятся друг за другом всегда и без каких либо условий. Рассмотрим пример.
...
Возможно, вы спросите: отличается ли такое поведение от конвейера? В обоих случаях получается линейный алгоритм, верно? В нашем примере с копированием каталогов разницы никакой нет, потому что утилита `cp` игнорирует входные данные с потока ввода. Следующая команда реализует тот же самый алгоритм:
{line-numbers: false, format: Bash}
```
cp -R ~/docs ~/docs-backup | cp -R ~/photo ~/photo-backup
```

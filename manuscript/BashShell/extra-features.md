...
|  | | |
| `sort` | Сортировка строк в потоке ввода в | `cat file.txt | sort -r` |
| | прямом и обратном (параметр `-r`) порядке. | `cat file.txt | sort -r` |
|  | | |
| `wc` | Подсчёт строк (параметр `-l`), слов (`-w`), | `cat file.txt | wc -l` |
| | букв (`-m`) и байтов (`-с`) в указанном файле или потоке ввода. | `info cat | wc -m` |
|  | | |
| `head` | Вывести указанное число байтов | `info cat | head -n 10` |
| | (параметр `-c`) или строк (`-n`) файла или текста из начала потока ввода. | `info cat | head -10` |
|  | | |
| `tail` | Вывести указанное число байтов | `info cat | tail -n 10` |
| | (параметр `-c`) или строк (`-n`) файла или текста из конца потока ввода. | `info cat | tail -10` |
|  | | |

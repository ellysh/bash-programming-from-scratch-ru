...
Вместо временных файлов передавайте данные через [**конвейеры**](https://ru.wikipedia.org/wiki/Конвейер_(Unix)) (pipeline). Конвейер — это механизм взаимодействия процессов. Он передаёт сообщения без использования файловой системы.

Рассмотрим пример. Предположим, что мы ищем информацию о [лицензии](https://ru.wikipedia.org/wiki/Лицензия_на_программное_обеспечение) в документации по Bash. Для этого применим утилиту grep. Выполним следующую команду в каталоге с документацией:
{line-numbers: false, format: Bash}
```
...
2. Если есть непосещённый подкаталог, перейти в него и начать с пункта 1 алгоритма.
...
По умолчанию утилита sort сортирует строки в порядке возрастания по алфавиту. Это называется [**лексикографической сортировкой**](https://ru.wikipedia.org/wiki/Лексикографический_порядок). Например, есть файл со следующими строками:
...
I> Эта команда не обработает файлы, имена которых содержат пробелы и символы перевода строки. Решение этой проблемы приводится в следующем разделе.
...
|  | | |
| `tee` | Перенаправляет поток ввода одновременно в поток вывода и в файл. | `grep "GNU" file.txt | tee result.txt` |
|  | | |
...
Стандарт POSIX разрешает любые символы в именах файлов, в том числе пробелы и перевод строки. Единственный запрещённый символ — это [**нуль-терминатор**](https://ru.wikipedia.org/wiki/Нуль-терминированная_строка) (NULL). Рассмотрим последствия этого правила.
...
Предположим, что нам нужен более сложный алгоритм. В нём результат выполнения первой команды определяет следующий шаг. Если команда выполнилась успешно, требуется одно действие. В противном случае — другое. Про такой алгоритм говорят, что он содержит [**ветвление**](https://ru.wikipedia.org/wiki/Ветвление_(программирование)). Сам алгоритм называется **разветвляющимся**.
...
Стандарт POSIX требует, чтобы каждая запущенная программа при завершении выдавала [**код возврата**](https://ru.wikipedia.org/wiki/Код_возврата) (exit status). При успешном завершении этот код равен нулю. В противном случае он принимает любое значение от 1 до 255.
...
I> Код возврата последней выполненной команды всегда сохраняется в переменной окружения Bash с именем `?`. Для вывода её значения используйте команду echo. Например:
{line-numbers: false, format: Bash}
```
echo $?
```
...
В Bash приоритет операторов && и || одинаковый. Выражение вычисляется слева направо. В таком случае говорят, что операторы [**левоассоциативны**](https://ru.wikipedia.org/wiki/Очерёдность_операций). Учитывая это, перепишем наше выражение в следующем виде:
{line-numbers: false}
```
(A && B) || C
```

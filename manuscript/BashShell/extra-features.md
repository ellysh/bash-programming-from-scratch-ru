...
Здесь используется встроенная команда интерпретатора `echo`. Ей на вход передаётся результат выполнения команды `0< README.txt`, то есть вывод содержимого файла `README.txt` на стандартный поток ввода. В этом примере выполняется **подстановка команд**. Она заключается в том, что интерпретатор встретив символы `$(` и `)` выполняет заключенную между ними команду и подставляет её вывод.

При подстановке команд всегда важно учитывать порядок их исполнения. Он соответствует порядку их следования. Только после выполнения всех подстановок интерпретатор обработает получившуюся команду целиком. По этой причине следующий вызов `find` завершится с ошибками:
{line-numbers: true, format: Bash}
```
$ find / -path */doc/* -name README -exec $(0< {}) \;
bash: {}: No such file or directory
find: invalid argument `;' to `-exec'
```

Дело в том, что команда `0< {}` будет исполнена перед вызовом утилиты `find`. В результате её выполнения содержимое файла с именем `{}` будет перенаправлено на стандартный поток ввода. Но файла с таким именем не существует. Мы ожидали, что вместо символов `{}` произойдёт подстановка результатов поиска `find`. Однако, утилита `find` будет вызвана только после исполнения команды `0< {}`.

Чтобы вывести содержимое файлов, найденных `find`, необходимо использовать утилиту `cat`. Например, следующим образом:
{line-numbers: false, format: Bash}
```

...
Расмотрим подробнее каждый из этих механизмов.
...
Другое решение заключается в использоании файловой системы. Её можно использовать для временного хранения данных. Это намного удобнее буфера обмена. Во-первых снимается ограничение на количество строк. Во-вторых не требуются ручные операции копирования в буфер.
...
Полная форма команды перенаправления стандартного вывода с записью в конец файла выглядит как `1>>`. Точно так же можно перенаправить и поток ошибок: `2>>`. Если вам нужно перенаправить оба этих потока в один файл, воспользуйтесь командой `&>` или `&>>`:
{line-numbers: false}
```
...
Такой вызов завершится ошибкой от утилиты `cp`, потмоу что она не читает стандартный поток ввода, а ожидает имя копируемого файла в первом параметре. Наша задача заключается в том, чтобы сконструировать строку параметров из данных, полученных на стандартном потоке ввода. Утилита `xargs` делает именно то, что нам нужно:
{line-numbers: false}
```

...
Конвейеры позволяют сочетать команды и получать так называемые **линейные алгоритмы**. В таких алгоритмах действия выполняются последовательно друг за другом без каких либо условных операторов. Но предположим, что в нашем алгоритме результат выполнения первой команды определяет следующий шаг. Если команда выполнилась успешно, требуется одно действие, а в противном случае - другое. Про такие алгоритмы говорят, что они содержат **ветвление**.
...
Но после её выполнения строка "OK" будет записываться в файл `result.log` независимо от результата копирования. Даже если каталога `docs` не существует, вы получите сообщение об успешной операции в лог-файле.
...
В результате мы получим нужное нам поведение. Если утилита `cp` завершится с ошибкой, будет выведена строка `Error`. Почему? Чтобы понять причину такого поведения, нам следует разобраться с приоритетами операций.
...
Действительно, добавление скобок ничего не изменило. По прежнему сначала будет вычислено выражение `A && B`, а затем `C`, но только при необходимости.

Итак, что произойдёт если `A` равно истине? В этом случае оператор `&&` вычислит свой правый операнд B. Это приведёт к выводу "OK" в лог файл. Далее интерпретатор Bash дойдёт до оператора `||`. В этот момент значение его левой части `A && B` уже известено. Оно равно истине. В этом случае для вычисления всего выражения с логическим ИЛИ нет необходимости знать правый операнд. Как вы помните, всё выражение истинно, если хотя бы один из операндов истиннен. Поэтому вывода "Error" в лог файл не произойдёт.
...
Предположим нам нужно скопировать не один каталог, а два. Для этого можно выполнить следующую команду:
{line-numbers: false}
```
cp -R ~/docs ~/docs-backup ; cp -R ~/photo ~/photo-backup
```

В результате выполнения этой команды утилита `cp` будет вызвана дважды независимо от результата копирования каталога `docs`. Даже если оно завершится ошибкой, каталог `photo` всё равно будет скопирован.

Возможно, вы спросите: отличается ли поведение коннектора `;` от конвейера? В данном случае разницы никакой нет, потому что утилита `cp` игнорирует входные данные с потока ввода. Следующая команда реализует тот же самый алгоритм копирования двух каталогов:
{line-numbers: false}
```
cp -R ~/docs ~/docs-backup | cp -R ~/photo ~/photo-backup
```

Но в общем случае отличие есть. Коннектор `;` позволяет выполнить две команды независимо друг от друга. Если же вы используете конвейер, возникает зависимость. Стандартный поток вывода первой команды переадётся на ввод второй. Это может изменить её поведение.
...
Параметр `-` утилиты `grep` добавляет данные со стандартного потока ввода в конец команды. Иллюстрация 2-25 демонстрирует результаты выполнения для обоих случаев.

{caption: "Иллюстрация 2-25. Результат выполнения команд с конвейером и коннектором", height: "50%"}
![Конвейер и коннектор](images/BashShell/pipe-vs-connector.png)

Заметьте, что даже поведение команды `ls` отличается. При использовании конвейера её результат не выводится на экран, а перенаправляется на вход утилиты `grep`. Та его обрабатывает и выводит строку:
{line-numbers: false}
```
(standard input):README
```

Это означает, что искомое слово "README" встретилось в данных, полученных из потока ввода stdin. Кроме того, `grep` обработает все файлы в текущем каталоге и найдёт "README" в одном из них с именем `xz.txt`.

В варианте команды с коннектором `;` утилита `ls` выведет свой результат на экран. После этого будет вызвана `grep`. Она обработает все файлы текущего каталога и поток ввода. Но в данном случае, данных на stdin не будет. Поэтому `grep` найдёт искомое слово "README" только в файле `xz.txt`.

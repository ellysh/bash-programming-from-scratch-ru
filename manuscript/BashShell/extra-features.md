...
Что такое оператор `&&`? На самом деле это логическая операция И. Вы можете возразить, что в данном случае речь идёт не о выражениях (условиях), а о командах Bash (действиях). Какой смысл выполнять логическую операцию над двумя действиями? Всё не так просто.

...
В качестве более сложного примера рассмотрим правильный способ вывода содержимого файла на экран:
{line-numbers: false, format: Bash}
```
echo $( 0< README.txt )
```

Здесь мы используем команду интерпретатора `echo`. Ей на вход мы передаём результат выполнения команды `0< README.txt`, то есть содержимое файла `README.txt`. В этом примере мы использовали **подстановку команд**. Она заключается в том, что интерпретатор встретив символы `$(` и `)` выполняет заключенную между ними команду и подставляет её вывод вместо неё.
...
Рассмотрим ещё один пример. Конвейеры часто и ошибочно используются для анализа вывода утилиты `ls`. Если вам надо получить список файлов, в именах которых встречается некоторый шаблон, лучше пользоваться командой `find`. Сочетание команд `ls` и `grep` мы рассмотрим для простоты и исключительно в образовательных целях. 

В результате выполнения следующей команды на экран будут выведены все файлы из каталога `/usr/share/doc/bash`, в именах которых встречается слово html:
{line-numbers: false, format: Bash}
```
ls /usr/share/doc/bash | grep html
```

W> Применяйте конвейеры для обработки результата `ls` только для вывода данных на экран в режиме командного интерпретатора. Не делайте этого в скриптах и командах манипуляции файлами, поскольку этот подход часто приводит к ошибкам.
...
|  | | |
| `grep` | Поиск по указанному шаблону. | `grep "GNU" file.txt` |
| | | `info find | grep -A 3 -B 3 GNU` |
|  | | |
...
| | прямом и обратном (параметр `-r`) порядке. | `cat file1.txt file2.txt | sort -r` |
|  | | |
...
| | (параметр `-c`) или строк (`-n`) файла или текста из начала потока ввода. | `info find | head -10` |
|  | | |
| `tail` | Вывести указанное число байтов | `tail -n 10 file.txt` |
| | (параметр `-c`) или строк (`-n`) файла или текста из конца потока ввода. | `info find | tail -10` |
|  | | |

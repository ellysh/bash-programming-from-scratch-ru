...
Рассмотрим ещё один пример. Конвейеры часто  и ошибочно используются для анализа вывода утилиты `ls`. Если вам надо получить список файлов, в именах которых встречается некоторый шаблон, лучше пользоваться командой `find`. Если же сочетать команды `ls` и `grep`, результат получится короче и удобнее для набора:
{line-numbers: false, format: Bash}
```

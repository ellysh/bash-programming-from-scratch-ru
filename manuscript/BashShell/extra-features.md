...
Мы уже знаем, как с помощью параметра `-exec` команды `find` выполнить действие над каждым найденным файлом. Возможно ли сделать подобное с применением конвейеров? Да, это возможно, но в общем случае лучше использовать `find`. Мы рассамотрим пример с конвейрами исключительно для образовательных целей.

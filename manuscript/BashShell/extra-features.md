...
Конвейеры позволяют вам сочетать команды и получать так называемые **линейные алгоритмы**. В таких алгоритмах действия выполняются последовательно друг за другом без каких либо условных операторов. 

Но предположим, что в нашем алгоритме результат выполнения первой команды определяет следующий шаг. Если команда выполнилась успешно, требуется одно действие, а в противном случае - другое. Про такие алгоритмы говорят, что они содержат **ветвление**.

Обратимся к примеру. Нам нужно написать команду для копирования каталога. Если она выполняется успешно, то в лог файл надо записать "OK", а в противном случае - "Error".

Используя конвейер, мы могли бы составить такую команду:
{line-numbers: false}
```
cp ~/docs ~/docs-backup | echo "OK" > result.log
```

Но после её выполнения в файл `result.log` строка "OK" будет записываться независимо от результата копирования. Даже если каталога `docs` не существует, вы получите сообщение об успешной операции в лог-файле.
...
Что такое коннектор `&&`? На самом деле это логическая операция И. Вы можете возразить, что в данном случае речь идёт не о выражениях (условиях), а о командах Bash (действиях). Какой смысл выполнять логическую операцию над действием? Всё не так просто.

Стандарт POSIX, которому следует Bash, требует, чтобы каждая запущенная программа завершалась с определённым [**кодом возврата**](https://ru.wikipedia.org/wiki/Код_ответа). Если этот код равен нулю, значит программа выполнилась успешно. В противном случае возвращаемое значение может принимать любое значение от 1 до 255. Когда вы применяете логический оператор к команде, на самом деле он вычисляет её код возврата.
...
Предположим, что утилита `cp` завершилась успешно. В этом случае она вернёт ноль. В Bash ноль соответствует значению "истина". Поэтому левая часть нашего оператора `&&` будет истинной. Чтобы вычислить значение всего выражения, оператору придётся запустить команду `echo` в правой части и получить её код возврата. `echo` всегда завершается успешно и возвращает код ноль. Таким образом результат работы оператора `&&` будет "истина". Как же мы используем этот результат? Ответ - никак.

Коннекторы являются логическими операторами, но обычно цель их использования вовсе не вычисление выражений. Они применяются ради своих побочных эффектов, а именно - порядка вычислений.

Рассмотрим ситуацию, когда утилита `cp` в нашей команде завершается с ошибкой. Тогда её код возврата будет отличен от нуля. Для Bash это эквивалентно значению "ложь". В этом случае оператор `&&` может вычислить значение всего выражения, зная только левый операнд. Действительно, если хотя бы один операнд логического ИЛИ ложный, всё выражение будет ложным независимо от правого операнда. Таким образом команда `echo` не будет выполнена и запись в файл `result.log` не произойдёт.

I> Код возврата последней выполненной команды всегда сохраняется в переменной окружения с именем `?`. Чтобы вывести её значение на экран, воспользуйтесь командой `echo`:
{line-numbers: false}
```
echo $?
```

Как вы помните, в условии нашей задачи сказано, что в случае ошибки в лог файл надо вывести строку "Error". Дополним нашу команду оператором ИЛИ, который в Bash обозначается как `||`:
{line-numbers: false}
```
cp ~/docs ~/docs-backup && echo "OK" > result.log || echo "Error" > result.log
```

В результате мы получим нужное нам поведение. Если утилита `cp` завершится с ошибкой, будет выведена строка `Error`. Чтобы понять причину такого поведения, нам следует разобраться с приоритетами операций.

Для простоты дальнейшего обсуждения обозначим код возврата утилиты `cp` как A. Первая команда `echo` вернёт нам код B, а вторая - C. Тогда нашу команду можно записать в такой форме:
{line-numbers: false}
```
A && B || C
```

Итак, что произойдёт если A равно истине? В этом случае оператор `&&` вычислит свой правый операнд B. Это приведёт к выводу "OK" в лог файл. Далее интерпретатор Bash дойдёт до оператора `||`. В этот момент значение его левого операнда `A && B` уже известено. Оно равно истине. В этом случае для вычисления всего выражения с логическим ИЛИ нет необходимости знать правый операнд. Как вы помните, всё выражение истинно, если хотя бы один из операндов истиннен. Поэтому вывода "Error" в лог файл не произойдёт.

Если A равно значению ложь, выражение `A && B` также будет ложным. При этом вычисление операнда B не требуется, а значит вывода "OK" в лог файл не будет. Когда Bash перейдёт к следующему далее оператору `||`, ему уже известно, что левый операнд ложный. Поэтому для вычисления всего выражения требуется узнать значение правого операнда. Это приведёт к исполнению второй команды `echo` и выводу строки "Error" в лог файл.

Логика работы операторов `&&` и `||` неочевидна и достаточно сложна для понимания. Возможно, вам понадобится какое-то время, чтобы её освоить.
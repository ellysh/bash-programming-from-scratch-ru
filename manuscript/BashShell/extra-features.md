...
Эта команда должна вывести содержимое всех файлов, найденных утилитой find. Но вместо этого мы получили ошибку от Bash.
...
Рассмотрим другой пример с конвейерами. GNU утилита du оценивает использование дискового пространства. Запустите её без параметров в текущем каталоге. Она **рекурсивно** пройдёт по всем его подкаталогам и выведет занимаемое ими пространство.
...
2. Если есть непосещённый подкаталог, перейти в него и начать с пункта 1.

3. Если все подкаталоги посещены или их нет, перейти на уровень выше и начать с пункта 1.

4. Если перейти на уровень выше нельзя, завершить алгоритм.

Следуя этому алгоритму, мы обойдём все подкаталоги от выбранной точки файловой системы. Это универсальный алгоритм обхода. К нему можно добавить любое действие над каждым найденным подкаталогом. В случае утилиты du это расчёт занимаемого дискового пространства. Тогда алгоритм утилиты будет выглядеть так:
...
Это таблица из двух столбцов. В правом указан подкаталог, а в левом — сколько байтов он занимает на диске. В этот вывод можно добавить статистику по файлам в подкаталогах. Для этого укажите опцию `-a`. Получится следующая команда:
{line-numbers: false, format: Bash}
```
du /usr/share -a
```
...
Почему в шаблоне `"\.html"` точка экранирована? Дело в том, что точка означает однократное вхождение любого символа. Если указать шаблон `".html"`, в вывод утилиты grep попадут лишние файлы (например, `pod2html.1perl.gz`) и подкаталоги (`/usr/share/doc/pcre/html`). Если точку экранировать, утилита grep обработает её как символ точка.
...
По умолчанию утилита sort сортирует строки по алфавиту ([**лексикографическая сортировка**](https://ru.wikipedia.org/wiki/Лексикографический_порядок)). В результате строки следуют по возрастанию. Например, у нас есть файл со следующими строками:
...
В выводе утилиты du в первом столбце указаны размеры файлов. Поэтому именно их будет сортировать sort. Размеры файлов — это числа. Из-за этого лексикографическая сортировка не подойдёт. Рассмотрим пример. У нас есть файл с числами:
{line-numbers: true, format: text}
```
3
100
...
Получилось, что число 100 меньше 2 и меньше 3. Причина в том, что ASCII-код символа 1 меньше чем символов 2 и 3. Чтобы отсортировать целые числа по их значению, используйте опцию sort `-n`.
...
Конвейеры можно сочетать с перенаправлением потоков. Если нужно сохранить отфильтрованный и отсортированный вывод утилиты du в файл, выполните следующую команду:
{line-numbers: false, format: Bash}
```
...
Сочетание конвейеров и перенаправления потоков может привести к проблеме. Что делать, если поток данных надо направить и в файл, и на вход утилите? У Bash нет встроенного механизма для этой задачи. Проблему можно решить GNU-утилитой tee. Рассмотрим пример.
...
Результат утилиты du будет выведен на экран. Этот же результат будет записан в файл `result.txt`. Таким образом утилита tee продублировала свой поток ввода в указаный файл и в поток вывода. При этом содержимое `result.txt` перезапишется. Чтобы вместо этого дописать вывод tee в конец файла, используйте опцию `-a` утилиты.

Предположим, вам нужно проверить поток данных между командами в конвейере. Утилита tee позволяет это сделать. Для этого добавьте её вызов между этими командами. Например так:
{line-numbers: false, format: Bash}
```
du /usr/share -a -h | tee du.txt | grep "\.html" | tee grep.txt | sort -h -r > result.txt
```

Вывод каждой команды конвейера попадёт в соответствующий файл. Эти промежуточные результаты могут пригодиться для поиска ошибок. Конечный результат работы конвейера по прежнему будет записан в файл `result.txt`.

...
I> Использовать утилиту `cat` для вывода содержимого файла неправильно. На самом деле её назначение — объединять несколько файлов и выводить результат в стандартный поток вывода. Вместо вызова `cat` для одного файла следует использовать перенаправление потоков, которое мы рассмотрим далее в этой главе. Пример правильного вывода содержимого файла на экран:
...
После строки для поиска идут относительные или абсолютные пути к интересующим нас файлам, разделённые пробелом. В нашем примере передаётся только путь до `README` файла с документацией на Bash. Результат выполнения команды приведён на иллюстрации 2-17.

{caption: "Иллюстрация 2-17. Результат вызова утилиты `grep`", width: "100%"}
![Результат grep](images/BashShell/grep-command.png)

В вывод утилиты можно добавить номера строк файла, в которых встретилась искомая строка. Для этого добавьте опцию `-n`. Результат такого вызова приведён в нижней части иллюстрации 2-17.
...
Если сделать так, то подстановки Bash не будет и шаблон поиска будет передан как есть утилите `grep`. В отличие от `find` она не умеет самостоятельно разворачивать шаблоны. Поэтому будет выведено сообщение об ошибке как на иллюстрации 2-18.

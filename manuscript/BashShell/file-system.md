...
Командный интерпретатор позволяет запускать установленные программы. Но кроме этого он предоставляет встроенные команды. Они не требуют какого-то приложения, а исполняются самим интерпретатором. В таблице 2-1 приведены следующие команды:
...
При первом знакомстве с интерпретатором Bash разница между командами и утилитами не важна. Но в будущем при написании скриптов её следует учитывать. Мы будем сразу различать команды и утилиты.
...
Может возникнуть вопрос: что такое опция команды `-W`? Одна из проблем интерфейса командной строки в том, что пользователь ограничен в средствах взаимодействия с запускаемым приложением. Обычно режим его работы задаётся в момент запуска. Кроме того программе может понадобиться дополнительная информация (например, путь до файла или каталога). Самый простой способ передать эту информацию — поместить её в строку, следующую за именем приложения. Командная оболочка позаботится о том, чтобы программа её получила. Эта строка состоит из параметров команды.

[**Параметром**](https://ru.wikipedia.org/wiki/Интерфейс_командной_строки#Формат_команды) или аргументом команды называются слова или символы, которые передаются ей на вход. **Опцией** или ключём называется аргумент в стандартизированной форме (начинается с `-` или `--`), который переключает режим работы команды. 
...
После уточнения текущего каталога, будет полезно узнать его содержимое. Для этого воспользуемся командой `ls`. Её вывод для домашнего каталога `~` будет пустым, как в верхней части иллюстрации 2-8. Попробуем выполнить эту команду с опцией `-a`. Вы увидите восемь файлов, начинающихся с точки, как в нижней части иллюстрации 2-8.

{caption: "Иллюстрация 2-8. Вывод команды `ls`"}
![Вывод ls](images/BashShell/ls-command.png)

Файлы и каталоги являются скрытыми, если их имена начинаются с точки. Как правило, они создаются приложениями автоматически и хранят их конфигурацию. Опция `-a` команды `ls` позволяет вывести скрытые файлы и каталоги.
...
Когда к вашему компьютеру подключено несколько дисков, может быть полезно вывести на экран их точки монтирования. Эту задачу выполняет утилита [`mount`](https://ru.wikipedia.org/wiki/Mount), запущенная без параметров. Иллюстрация 2-10 демонстрирует результат её выполнения в моём случае.

{caption: "Иллюстрация 2-10. Вывод утилиты `mount`"}
...
Таблица 2-2 представляет собой вывод утилиты `mount` с иллюстрации 2-8, разделённый на столбцы.
...
Если вы воспользуетесь командой `ls`, то обнаружите несколько подкаталогов, среди которых есть `share`. В нём есть подкаталог `doc/bash`. Перейдите в него командой `cd share/doc/bash`. По `ls` вы обнаружите там файл `README`. Выведите его содержимое на экран утилитой `cat README`. Её результат приведён на иллюстрации 2-12.

{caption: "Иллюстрация 2-12. Вывод утилиты `cat`", height: "30%"}
...
{caption: "Иллюстрация 2-13. Результат работы утилиты `find`", height: "30%"}
...
{caption: "Таблица 2-4. Часто используемые действия утилиты `find`", width: "100%"}
...
I> При передаче шаблонов в утилиту `find` вы должны заключать их в двойные кавычки `"`. Они выполняют ту же функцию что и обратный слэш, экранирующий управляющие символы. Таким образом мы даём Bash понять, что ему не следует интерпретировать шаблоны самому (то есть выполнять подстановку), а вместо этого просто передавать их вызываемой утилите.
...
```
Напишите команду вызова find для поиска всех TXT файлов в вашем Linux окружении.

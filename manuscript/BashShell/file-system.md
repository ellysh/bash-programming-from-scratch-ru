...
Предположим, нас интересует список имён файлов, в которых встречается фраза. В обычном режиме утилита grep выводит все вхождения искомой фразы. Это лишний вывод, который не всегда нужен. Уберём его опцией `-l`. Например, так:
{line-numbers: false, format: Bash}
```

...
В этом алгоритме нет никаких условий. Он реализует простейшее поведение робота. Но представьте, что произойдёт если на пути робота застрял погрузчик. Если он будет следовать приведённому выше алгоритму, произойдёт столкновение. Чтобы этого избежать, мы должны добавить условие:
...
Логические операции появляются, когда возникает несколько выражений. Представьте, что в точке А может не оказаться коробки. Тогда роботу нет смысла двигаться в точку Б. В этом случае мы можем воспользоваться операцией [**логического И**](https://ru.wikipedia.org/wiki/Конъюнкция), известной так же как конъюнкция. Наш алгоритм станет выглядеть следующим образом:
...
Думаю, интуитивно понятно, что результат логического И будет истинным только тогда, когда оба условия истинны. То есть у робота есть коробка и нет препятствия. В любом другом случае он вычислит значение ложь и остановится.

Другие две логические операции, применяемые в программиовании, - это [ИЛИ](https://ru.wikipedia.org/wiki/Дизъюнкция) (дизъюнкция) и [НЕ](https://ru.wikipedia.org/wiki/Отрицание) (отрицание). На самом деле мы уже применили отрицание, когда написали выражение "нет препятствия". Мы можем переписать наш алгоритм более формально, явно указав НЕ:

1. Возьми коробку в точке А.

2. Если есть коробка И НЕ препятствия, двигайся в точку Б. Иначе остановись.

3. Положи коробку.

А теперь инвертируем наше выражение и применим логическое ИЛИ:

1. Возьми коробку в точке А.

2. Если НЕ коробка ИЛИ есть препятствие, остановись. Иначе двигайся в точку Б.

3. Положи коробку.

Очевидно, что логика робота нисколько не изменилась. Он по прежнему не будет двигаться в точку Б, если у него нет коробки или на пути возникло препятствие.

Этих минимальных знаний по логическим выражениям вам должно хватить на первое время.

...
После запуска приложения `msys2.exe` откроется окно [**эмулятора терминала**](https://ru.wikipedia.org/wiki/Эмулятор_терминала). [**Эмулятор**](https://ru.wikipedia.org/wiki/Эмуляция) — это программа, которая имитирует поведение другой программы, ОС или устройства для обеспечения совместимости. Например, [эмулятор MS-DOS](https://ru.wikipedia.org/wiki/DOSBox) позволяет запускать DOS-программы на Windows. Похожей цели служит и эмулятор терминала. Благодаря ему пользователи могут работать с приложениями, предназначенными только для работы через компьютерный терминал. Одним из таких приложений является интерпретатором командной строки.

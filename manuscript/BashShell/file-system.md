...
Команда выведет список всех файлов с документацией на всех смонтированных дисках. Этот список слишком длинный. Сократим его с помощью дополнительных условий поиска. Добавим строку `bash` в путь искомого файла. Получится следующая команда:
{line-numbers: false, format: Bash}
```
...
Передавая шаблоны в find, заключайте их в двойные кавычки ". Кавычки выполняют ту же функцию, что и обратный слэш перед круглыми скобками. Так мы запрещаем Bash интерпретировать шаблоны. Вместо этого утилита find должна получать их как есть и интерпретировать сама. 

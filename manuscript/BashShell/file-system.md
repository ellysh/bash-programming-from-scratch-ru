...
[**Каталог**](https://ru.wikipedia.org/wiki/Каталог_(файловая_система)) — это объект файловой системы, который может содержать файлы и другие каталоги. Каталоги нужны, чтобы группировать другие объекты. Это облегчает навигацию по файловой системе. В терминологии Windows каталоги называются [**папками**](https://ru.wikipedia.org/wiki/Каталог_(файловая_система)#Термин_«Папка»). Оба названия обозначают один и тот же объект файловой системы.
...
Первый параметр утилиты — это искомая строка. Не забывайте про двойные кавычки. Они гарантируют, что grep получит строку без изменений. Без кавычек Bash разделит её пробелом на два отдельных параметра. Этот механизм разделения строк называется [**word splitting**](http://mywiki.wooledge.org/WordSplitting).
...
Если вы работаете на Linux или macOS предпочитайте опцию `-R`. С ней утилита grep будет переходить по [**символическим ссылкам**](https://ru.wikipedia.org/wiki/Символическая_ссылка) во время поиска. Вот пример использования этой опции:
{line-numbers: false, format: Bash}
```
grep -R "free software" .
```
...
{caption: "Таблица 2-6. Утилиты для работы с PDF и MS Office файлами", width: "70%", column-widths: "20% *"}
| Утилита | Функции |
| --- | --- |
| [pdftotext](http://www.xpdfreader.com) | Конвертирует PDF-файл в текстовый формат. |
|  | |
| [pdfgrep](https://pdfgrep.org) | Ищет PDF-файл по его содержимому. |
|  | |

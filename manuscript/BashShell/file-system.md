...
I> Важная особенность POSIX совместимых систем заключается в [чувствительности к регистру символов](https://ru.wikipedia.org/wiki/Чувствительность_к_регистру_символов). Это означает, что строки `Data` и `data` различаются. В Windows чувствительности к регистру нет. Поэтому вы можете задать в адресной строке Проводника путь `c:\windows`, в результате чего будет открыт системный каталог `Windows`. В Linux и Bash это не сработает. Вам нужно вводить все символы пути в правильном регистре. Так же обратите внимание, что в Bash в качестве разделителя между именами каталогов и файлов спользуется символ [**слэш**](https://ru.wikipedia.org/wiki/Косая_черта) `/`, а не [**обратный слэш**](https://ru.wikipedia.org/wiki/Обратная_косая_черта) `\`, как в Windows.
...
| | каталог не указан, быдет выведено содержимое текущего. | `ls /c/Windows` |
| _ | | |
...
| _ | |
| ИЛИ | Истинно, когда любой из операндов истиннен. Ложно только тогда, когда все операнды ложны. |
| _ | |
...
Поробуйте оба варианта вызова `grep`: с шаблоном `*` и явным указанием файлов. Вы получите одинаковый результат.
...
Во втором способе поиска с помощью утилиты `grep` используется её функия перебора файлов в указанном каталоге. Чтобы включить эту функцию, укажите параметр `-r`. При этом последним параметром нужно указать путь к каталогу:
{line-numbers: false}
```

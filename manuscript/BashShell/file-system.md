...
После запуска приложения `msys2.exe` откроется окно [**эмулятора терминала**](https://ru.wikipedia.org/wiki/Эмулятор_терминала). [**Эмулятор**](https://ru.wikipedia.org/wiki/Эмуляция) — это программа, которая имитирует поведение другой программы или устройства для обеспечения совместимости. Этой же цели служит и эмулятор терминала. Благодаря ему пользователи могут работать с приложениями, предназначенными для работы через устройство терминал. Одним из таких приложений является интерпретатором командной строки.

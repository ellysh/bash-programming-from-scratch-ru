...
I> Почему скобки, передаваемые в команду `find`, должны экранироваться обратным слэшем? Дело в том, что для языка Bash это [управляющие символы](https://ru.wikipedia.org/wiki/Управляющие_символы), т.е. они имеют некоторое синтаксическое значение. Поэтому Bash обработает их сам и не передаст на вход утилиты `find`. Чтобы этого не происходило, скобки нужно экранировать.

...
В POSIX-совместимых системах [важен регистр символов](https://ru.wikipedia.org/wiki/Чувствительность_к_регистру_символов). Это значит, что строки `Documents` и `documents` не равны. В Windows нет чувствительности к регистру. Поэтому если в адресной строке Проводника написать путь `c:\windows`, вы перейдёте в системный каталог `C:\Windows`. В Unix-окружении это не сработает. Все символы надо вводить в правильном регистре. Кроме того в Unix имена каталогов и файлов в пути разделяет слэш /. В Windows для этого используют обратный слэш \.
...
* Поиск каталога или файла на диске.
...
| --- | --- | --- |
| `ls` | Вывести на экран содержимое указанного каталога. | `ls` |
| | Если каталог не указан, будет выведено содержимое текущего. | `ls /c/Windows` |
|  | | |

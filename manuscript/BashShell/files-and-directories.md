...
I> Многие опции GNU утилит имеют краткую и полную форму. Краткая форма начинается с тире `-` и состоит из одной буквы. Полная форма начинается с двойного тире и представляет собой слово или несколько слов, разделённых тире.
...
Теперь мы легко можем прочитать права доступа к файлам `report.txt` и `report1.txt` на иллюстрации 2-25. Первый из них могут читать все пользователи, но его изменение и исполнение запрещено для всех. Второй файл могут читать все, а изменять только владелец. При этом его исполнение также запрещено для всех.

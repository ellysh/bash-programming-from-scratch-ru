...
В таблице 2-6 приводятся GNU утилиты, соответствующие этим действиям.

{caption: "Таблица 2-6. GNU утилиты для работы с файлами и каталогами", width: "100%"}
...
Кратко рассмотрим эти команды. Теперь вы знаете, как искать информацию о GNU утилитах и функциях Bash. Поэтому если возникнут вопросы, вам будет просто найти на них ответы.

### mkdir

Команда `mkdir` создаёт новый каталог. Первым параметром она принимает его абсолютный или относительный путь. Например, чтобы создать каталог `docs` в домашнем каталоге пользователя выполните команду:
{line-numbers: false}
```
mkdir ~/docs
```

Вы также можете создать `docs` по относительному пути. Для этого сначала перейдите в домашний каталог пользователя:
{line-numbers: false}
```
cd ~
mkdir docs
...
Также можно воспользоваться шаблоном поиска. Следующая команда удаляет все TXT файлы начинающиеся со слова "report":
{line-numbers: false}
```
rm ~/docs/reports/2019/report*.txt
```

Если вы попробуете удалить файл с доступом только на чтение, будет выведено предупреждение как на иллюстрации 2-25.

{caption: "Иллюстрация 2-25. Предупреждение при удалении файла с доступом на чтение", width: "100%"}
![Предупреждение `rm`](images/BashShell/rm-warning.png)

Чтобы продолжить удаление, нажмите клавишу y (от yes), а затем Enter. Если вы хотите проигнорировать подобные предупреждения, воспользуйтесь параметром `-f` или `--force`:
{line-numbers: false}
```
...
Команды копирования и переименования файлов и каталогов однотипны. В обеих сначала указывается то, над чем совершается действие, а потом путь назначения.
...
Если старый файл `report.txt` вам больше не нужен, воспользуйтесь командой переименования:
{line-numbers: false}
```
mv report.txt report-2019.txt
```

В этом случае будет создан новый файл `report-2019.txt`, а старый `report.txt` будет удалён.

Вы можете указывать абсолютные пути до файлов. Таким образом они будут копироваться или переносится между разными каталогами:
{line-numbers: false}
```
...
```
Упорядочьте свои фотографии за последние три месяца с помощью изученных команд Bash. Перед началом работы обязательно сделайте их резервную копию. Разделите все фотографии по годам и месяцам. Структура каталогов должна получиться примерно следующая:
...
При знакомстве с утилитой `rm` мы столкнулись с понятием [**прав доступа**](https://ru.wikipedia.org/wiki/Права_доступа) к файлу. Удаляя файл, доступный только для чтения, `rm` выводит предупреждение. Что означают эти права?
...
На иллюстрации 2-25 обратите внимание на команду `ls`, запущенную с параметром `-l`. Она выводит таблицу с подробной информацией о файлах и каталогах. Каждому из них соответствует одна строка таблицы. В первом её столбце указаны права доступа. Для файла `report.txt` они равны `-r--r--r--`. Рассмотрим, что означает эта запись.
...
Набор этих признаков можно закодировать маской из трёх битов. Допустим, что к файлу есть полный доступ и его можно читать, писать и исполнять. Тогда маска выглядит так:
{line-numbers: false}
```
111
```
...
Что означают буквы в строке прав доступа? Как мы выяснили, это равные единице биты. Позиция каждого из них определяет, какое действие над файлом или каталогом разрешено. Буквы были введены, чтобы упростить чтение битовой маски. Согласитесь, что понять строку `-rw-r--r--` проще и быстрее, чем двоичное число 0000011001000100. Значение каждого символа приведено в таблице 2-8.
...
Теперь если мы вернёмся к иллюстрации 2-25, мы легко сможем прочитать права доступа к файлам `report.txt` и `report1.txt`. Первый из них могут читать все пользователи, но его изменение и исполнение запрещено. Второй файл могут читать все, а изменять только владелец. При этом его исполнение также запрещено.
...
I> В Unix нет общепринятого соглашения о расширении исполняемых файлов. Поэтому, чтобы узнать их тип, вам пригодится команда `file`. Она принимает на вход относительный или абсолютный путь к проверяемому файлу:
{line-numbers: false}
```
file /usr/bin/ls
file ls
...
Мы уже знаем, что все GNU утилиты находятся в каталогах `/bin` и `/usr/bin`. Bash хранит их пути в переменной `PATH`. Может возникнуть вопрос: куда устанавливаются новые приложения в Linux окружении?

В Windows на системном диске есть каталоги `Program Files` и `Program Files (x86)`. По умолчанию все приложения устанавливаются туда. Для каждого из них создаётся новый каталог (например, `C:\Program Files (x86)\Notepad++`). В процессе установки в него копируются исполняемые файлы, DLL библиотеки, файлы конфигурации и ресурсов. Всё это вместе с системными библиотеками Windows необходимо для запуска и работы приложения. Вместо каталогов `Program Files` пользователь может указать любой другой путь установки (например, `D:\Programs`).
...
Первый вариант нам хорошо знаком. Именно таким образом мы вызывали GNU утилиты до сих пор. Например, запуск `find` из каталога `/usr/bin`:
{line-numbers: false}
```
find --help
```

...
[**Shell Variables**](http://tldp.org/LDP/Bash-Beginners-Guide/html/sect_03_02.html) (переменные оболочки) — переменные, которые устанавливаются интерпретатором. Они нужны для его корректной работы. Доступны пользователю для чтения и записи. Выводятся встроенной командой `set`, вызванной без параметров. Пример — переменная `PATH`. Иногда используется, как синоним термина Environment Variables (переменные окружения), но это неправильно.

...
ОС не только управляет ресурсами, но и организует работу запущенных приложений. Запуск приложения — это нетривиальная задача. За её выполнение отвечают специальные служебные программы ОС. После запуска приложения, ОС контролирует его выполнение. Если нарушается какое-то ограничение, приложение завершается. Пример нарушения — чтение недоступной памяти. В следующем разделе мы рассмотрим процесс запуска и исполнения программы.

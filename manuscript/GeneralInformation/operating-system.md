...
Пример современного интерфейса командной строки приведён на иллюстрации 1-7. Это окно эмулятора терминала [Terminator](https://en.wikipedia.org/wiki/Terminator_(terminal_emulator)), в котором запущен интерпретатор командной строки Bash. В окне выведены результаты работы программ ping и ls.
...
Сегодня можно наблюдать синтез культур Windows и Linux. Всё больше коммерческих приложений портируются на Linux. При этом их разработчики не всегда готовы вносить изменения, продиктованные Linux-культурой. Такие изменения требуют времени и сил. Кроме того они усложняют сопровождение продукта, потому что вместо одного приложения под разные платформы получается два. Поэтому всё чаще на Linux встречаются приложения, разработанные в типичном для Windows стиле. Это однозначно идёт на пользу платформе. Чем больше приложений запускается на ОС, тем популярнее она становится благодаря сетевому эффекту.

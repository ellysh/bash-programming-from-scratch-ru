...
На смену табуляторам, работающим на вращающихся цилиндрах, пришли компьютеры, использующие на [**реле**](https://ru.wikipedia.org/wiki/Реле). Реле — это механический элемент, который меняет своё состояние под воздействием электрического тока. Один из [первых релейных компьютеров](https://habr.com/ru/company/ua-hosting/blog/386247), называвшийся Z2, был сконструирован немецким инженером [Конрадом Цузе](https://ru.wikipedia.org/wiki/Цузе,_Конрад) в 1939 году. Затем компьютер был усовершенствован в 1941 году и получил название Z3. Благодаря переходу на реле удалось сократить время одной операции с секунд до миллисекунд.

Кроме возросшей скорости вычислений, компьютеры Цузе отличала ещё одна особенность. В них появилось понятие программы. Алгоритмы для вычислений вводились с помощью перфокарт, а исходные данные для них пользователь набирал на клавиатуре.

Появление [**программируемых компьютеров**](https://ru.wikipedia.org/wiki/Компьютер_общего_назначения) (также известных как универсальные) стало важной вехой в развитии вычислительной техники. До этого момента машины были способны выполнять только узкоспециализированные задачи. Часто это было неэффективно.

...
Вернёмся к нашему вопросу о необходимости ОС. Мы пришли к выводу, что она совершенно необязательна для запуска приложений на современном персональном компьютере. Такие программы есть и широко распространены. Например, [**утилиты**](https://ru.wikipedia.org/wiki/Утилита) проверки памяти, разбивки диска и некоторые антивирусы. Но их разработка обходится дорого, а запуск неудобен для пользователя, поскольку требует перезагрузки компьютера. Поэтому большинство программ пишется под ОС и использует её возможности.
...
Иллюстрация 1-4 демонстрирует первый персональный компьютер IBM 5150, получивший массовое распространение.

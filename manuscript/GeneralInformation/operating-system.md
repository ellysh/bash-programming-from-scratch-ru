...
Впервые математическую модель универсального компьютера предложил Алан Тьюринг в 1936 году. Впоследствии эта модель стала известна как [**машина Тьюринга**](https://ru.wikipedia.org/wiki/Машина_Тьюринга). Она описывает абстрактный автомат, который способен выполнить любой [**алгоритм**](https://ru.wikipedia.org/wiki/Алгоритм).
...
Мы рассмотрели ОС в общем. Теперь с учётом наших новых знаний поговорим о современных ОС. Они отличаются не столько в предоставляемых функциях, которые у всех почти одинаковы, сколько в способе их реализации. Эти особенности реализации и решения, которые к ним привели, принято называть [**архитектурой**](https://ru.wikipedia.org/wiki/Архитектура_программного_обеспечения).

Прежде всего следует заметить, что все современные ОС [**многозадачные**](https://ru.wikipedia.org/wiki/Многозадачность). Это значит, что они поддерживают одновременное исполнение программ. Почему так получилось?

ОС, которые использовались на первых [персональных компьютерах](https://ru.wikipedia.org/wiki/Персональный_компьютер#История), были однозадачными. Это всех устраивало, поскольку производительность таких компьютеров была значительно ниже чем у [мэйнфреймов](https://ru.wikipedia.org/wiki/Мейнфрейм), применявшихся в коммерческих целях и государственных проектах. То есть об эффективном одновременном выполнении нескольких программ тогда не было и речи. Возможно, некоторые читатели вспомнят [MS-DOS](https://ru.wikipedia.org/wiki/MS-DOS). Это хороший пример однозадачной ОС.
...
Представьте, что вы работаете над электронной таблицей. Например, заносите список сотрудников вашей компании и их контактные данные. Как бы быстро вы не печатали текст, между нажатиями клавиш проходят миллисекунды. По компьютерным масштабам это огромные промежутки времени, за которые процессор может обработать немало информации. Теперь представьте, что вы забыли чей-то номер и уточняете его у кого-то. Теперь счёт пошел на секунды и минуты. За это время современный процессор мог бы обработать бухгалтерский отчёт вашей фирмы. Мы рассмотрели только задержки ввода информации, но то же самое происходит и с выводом. Представтье, что на экран вывелось сообщение об ошибке. Несколько сотен миллисекунд у вас уйдёт только на то, чтобы понять, что произошло. Затем вы читаете сообщение. Думаю, вы уже поняли основную идею - пользователь очень медленно реагирует на события в масштабах компьютерного времени.

Чтобы процессор не простаивал, ОС может остановить выполнение приложения, ожидающего действия от пользователя, и предоставить аппаратные ресурсы другой программе, работающей параллельно. Именно в этом суть многозадачности.

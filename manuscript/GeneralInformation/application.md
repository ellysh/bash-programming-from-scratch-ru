...
В современном компьютере есть [несколько уровней памяти](https://ru.wikipedia.org/wiki/Иерархия_памяти). Они изображены на иллюстрации 1-7.

{caption: "Иллюстрация 1-7. Уровни памяти персонального компьютера"}
![Уровни памяти](images/GeneralInformation/memory-levels.png)

Стрелки на иллюстрации 1-7 соответствуют потокам данных. Их загрузка и выгрузка возможна только между соседними уровнями памяти. Другими словами если процессору нужны данные с дисковой памяти, они будут сначала загружены в оперативную память, затем в кэш и только потом попадут в регистры. Аналогичным образом происходит и обратный процесс записи данных на диск.

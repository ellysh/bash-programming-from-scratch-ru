...
Прежде чем познакомиться с памятью компьютера, будет полезно разобраться в единицах её измерения — [**байтах**](https://ru.wikipedia.org/wiki/Байт). Байт — это минимальный блок информации, на который может ссылаться процессор. Информация меньшего объёма просто не может быть сохранена независимо в памяти компьютера.

В то же время процессор может оперировать меньшими объёмами информации чем один байт. [Битом](https://ru.wikipedia.org/wiki/Бит) называется минимальная единица информации, которая дальше не может быть разложена на составные части. Логически один бит соответствует состоянию электрического сигнала: он есть или его нет. Таким образом, бит принимает два возможных значения: 0 или 1. Восемь битов составляют блок в один байт.

Часто используемые единицы объёма информации приведены в таблице 1-1.

{caption: "Таблица 1-1. Единицы объёма информации", width: "100%"}
...
{caption: "Таблица 1-2. Устройства хранения информации", width: "100%"}
...
Таблица 1-2 демонстрирует соответствие чисел в двоичной (binary, BIN), десятичной (decimal, DEC) и шестнадцатеричной (hexadecimal, HEX) системах счисления.
...
Чтобы перевести число из BIN в HEX достаточно разбить его на группы по четыре разряда, начиная с конца. Если последняя группа оказалась меньше четырёх разрядов её надо дополнить нулями впереди. Затем по таблице 1-2 каждую четвёрку заменяем на шестнадцатеричное число. Пример перевода:
```

...
Поскольку первые компьютеры использовались для вычислений, они оперировали целыми и дробными числами. Одного бита было недостаточно, чтобы закодировать число больше единицы. Поэтому возникла потребность объединять биты в группы. Такое объединение отразилось на архитектуре процессоров. Ожидалось, что большая часть операций компьютера будет происходить над группами битов. Поэтому намного эффективнее загружать их и обрабатывать как единое целое.
...
| [Жёсктий диск](https://ru.wikipedia.org/wiki/Жёсткий_диск) | до 16 Тбайт |
...
Программа, загруженная в оперативную память и исполняемая CPU, называется [**вычислительным процессом**](https://ru.wikipedia.org/wiki/Процесс_(информатика)) (process). Ещё раз обратите внимание на различие между программой и процессом. Первое представляет собой набор инструкций, а второе - непосредственное их исполнение.
...
Чтобы перевести число из BIN в HEX достаточно разбить его на группы по четыре разряда, начиная с конца. Если последняя группа оказалась меньше четырёх разрядов её надо дополнить нулями впереди. Затем по таблице 1-4 каждую четвёрку заменяем на шестнадцатеричное число. Пример перевода:
```
...
Вернёмся к нашему исполняемому файлу. Кроме него в оперативную память загружаются все необходимые для работы приложения библиотеки (в том числе и системные). За эту процедуру отвечает [**загрузчик программ Windows**](https://ru.wikipedia.org/wiki/Загрузчик_программ). Только после этого программа становится процессом и исполняется CPU, начиная с её первой инструкции. Когда программа вызывает функцию из какой-то библиотеки, соответствующий код уже находится в оперативной памяти. Благодаря этому, удаётся избежать простоев процессора, связанных с чтением данных с жёсткого диска.

После окончания работы программы занимаемая ей и загруженными библиотеками область RAM очищается и используется для другого приложения.

### Исходный код

Очевидно, для человека неудобно и трудоёмко писать программу в двоичных кодах. Особенно остро эта проблема начала проявляться с увеличением мощности компьютеров и усложнении программ для них. Для решения этой проблемы появились специальные приложения: [**компиляторы**](https://ru.wikipedia.org/wiki/Компилятор) и [**интерпретаторы**](https://ru.wikipedia.org/wiki/Интерпретатор). Они решают одну и ту же задачу: прочитать текст программы, написанный на каком-то [**языке программирования**](https://ru.wikipedia.org/wiki/Язык_программирования) и перевести его на язык, понятный процессору. Этот текст программы называется [**исходным кодом**](https://ru.wikipedia.org/wiki/Исходный_код).

Подход компиляторов и интерпретаторов различается. Ключевое отличие заключается в моменте генерации машинного кода из исходного. Компиляторы генерируют инструкции процессора заранее и сохраняют результат на жёсткий диск. Интерпретаторы же выполняют эту генерацию в оперативной памяти непосредственно в момент исполнения программы.
...
Иллюстрация 1-13 демонстрирует процесс компиляции программы.
...
Из иллюстрации видно, что процесс компиляции на самом деле состоит из двух этапов. Первый этап выполняется компилятором. Второй этап выполняется [**компоновщиком**](https://ru.wikipedia.org/wiki/Компоновщик) и называется **линковка**.

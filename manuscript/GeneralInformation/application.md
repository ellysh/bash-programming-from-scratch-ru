...
Остаётся открытым ещё один вопрос. Почему было выбрано именно восемь битов для объединения в байт? В ранних компьютерах размер байта был [шесть битов](https://ru.wikipedia.org/wiki/Шестибитные_кодировки). Это число было выбрано для кодирования всех символов английского алфавита в верхнем и нижнем регистре, цифр, знаков пунктуации и математических операций. Скоро этого размера стало не хватать и байт был расширен до семи битов. Этот момент совпал с появлением [ASCII-таблицы](https://ru.wikipedia.org/wiki/ASCII), которая стала стандартом для кодирования символов. После этого IBM выпустила мейнфрейм [IBM System/360](https://ru.wikipedia.org/wiki/IBM_System/360), который стал очень популярен и получил широкое распространение. В нём размер байта составлял восемь битов. Постепенно это стало стандартом в отрасли.

...
Таблица 1-2 демонстриурет соответствие чисел в двоичной (BIN), десятичной (DEC) и шестнадцатеричной (HEX) системах счисления.
...
После окончания работы программы, занимаемая ей и необходимыми ей библиотеками область RAM очищается и используется для другого приложения.
...
Теперь допустим, что для исполнения программы вы выбираете интерпретатор. В этом случае файл с исходным кодом уже готов для исполнения. Чтобы его исполнить, ОС сначала запускает интерпретатор. Далее интерпретатор прочитает файл с исходным кодом с диска в оперативную память и начнёт его выполнение строка за строкой. При этом преобразование каждой команды исходного кода в машинный код происходит в оперативной памяти. В целях оптимизации некоторые интерпретаторы сохраняют на жёсткий диск файлы с промежуточным представлением программы. Но главная идея остаётся неизменной - для исполнения программы всегда нужен интерпретатор.

Не вызывает сомнений, что процесс интерпретации значительно замедляет работу программы. Вместо её непосредственного исполнения на процессоре, появляется посредник. Получается, что компьютер должен предоставлять ресурсы не только вашему приложению, но и его интерпретатору. Однако, эта плата за удобство разработки программ зачастую приемлема. Как правило, язык программирования намного удобнее и выразительнее двоичного кода. Благодаря чему, написанные на нём программы намного проще (а значит и дешевле) разрабатывать и поддерживать.

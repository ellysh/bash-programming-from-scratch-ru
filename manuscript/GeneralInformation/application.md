...
Мы рассмотрели функции ОС. Теперь познакомимся с компьютерной программой. Что она из себя представляет? Как происходит её запуск и исполнение? Рассмотрим эти вопросы подробнее.

### Память компьютера

Прежде чем познакомиться с памятью компьютера, будет полезно разобраться в единицах её измерения — [**байтах**](https://ru.wikipedia.org/wiki/Байт). Байт — это минимальный блок информации, на который может ссылаться процессор и загружать в свою память. В то же время процессор может оперировать меньшими объёмами информации чем один байт. [**Бит**](https://ru.wikipedia.org/wiki/Бит) — это минимальная единица информации, которая не может быть разложена на составные части. Логически один бит соответствует состоянию электрического сигнала: он есть или его нет. Таким образом, бит принимает два возможных значения: 0 или 1. Восемь битов составляют блок в один байт.
...
Остаётся открытым ещё один вопрос. Почему было выбрано именно восемь битов для объединения в байт? В ранних компьютерах размер байта был [шесть битов](https://ru.wikipedia.org/wiki/Шестибитные_кодировки). Это число было выбрано для кодирования всех символов английского алфавита в верхнем и нижнем регистре, цифр, знаков пунктуации и математических операций. Скоро этого размера стало не хватать и байт был расширен до семи битов. Этот момент совпал с появлением [ASCII-таблицы](https://ru.wikipedia.org/wiki/ASCII), которая стала стандартом для кодирования символов. После этого IBM выпустила мэйнфрейм [IBM System/360](https://ru.wikipedia.org/wiki/IBM_System/360), который стал очень популярен и получил широкое распространение. В нём размер байта составлял восемь битов. Постепенно это стало стандартом в отрасли.
...
Стрелки на иллюстрации 1-11 соответствуют потокам данных, загрузка и выгрузка которых возможна только между соседними уровнями памяти. Процессор может читать и писать только в свои регистры. Поэтому если ему нужны данные с дисковой памяти, они будут сначала загружены в оперативную память, затем в кэш CPU и только потом попадут в его регистры. Аналогичным образом происходит и обратный процесс записи данных на диск.

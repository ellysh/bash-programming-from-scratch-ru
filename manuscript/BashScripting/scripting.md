...
Что будет делать эта команда? Для простоты рассмотрения перепишем это выражение, заменив команды на буквы латинского алфавита:
{line-numbers: false}
```
B && O1 || F1 && C && O2 || F2
```

Буквы `B` и `C` обзначают команды `bsdtar` и `cp`. `O1` и`F1` - это вывод в лог-файл OK и FAIL соответственно для первой команды. Аналогично, `O2` и `F2` - это вывод для второй.
...
Такое вывод только запутает пользователя.

Причина нашего затруднения в том, что вызов `echo` F1 всегда возвращает истину. Это приводит к тому, что выражение `B && O1 || F1` истинно в случае ошибки `bsdtar` и дальнейшие команды продолжают исполняться. Всё что нам нужно сделать - это инвертировать результат вызова F1 с помощью логического НЕ, которое в Bash обозначается как `!`:
{line-numbers: false}
```
B && O1 || ! F1 && C && O2 || F2
```
...
Такое поведение лучше предыдущего. Теперь вызова `cp` и копирования пустого архива не происходит. Но представьте, что мы пишем команду в которой около 100 команд. Если ошибка произойдёт, например, на 50-ой, вывод всех оставшихся всё равно попадёт в лог-файл. Это затруднит поиск проблемы. Правильным поведением в этом случае было бы прекращение выполнения всей команды после первой же ошибки. Чтобы добиться этого поведения, нам нужно сгруппировать каждую команду и её вывод следующим образом:
{line-numbers: false}
```
(B && O1 || ! F1) && (C && O2 || F2)
```

Рассмотрим, что произойдёт если `B` ложно. В этом случае отработает `F1` и вернёт ложь. В результате всё выражение `(B && O1 || ! F1)` будет ложным. Тогда благодаря короткому замыканию, коннектор `&&` не станет вычислять свой правый операнд, равный выражению `(C && O2 || F2)`. Мы получили именно то поведение, которое нам было нужно.

Добавим последний штрих. Имеет смысл инвертировать последний вызов `echo` F2. Благодаря этому, наша команда будет ложной, если операция `B` истинна, а `C` ложна. Такое поведение может быть полезно, в случае интеграции нашей команды с дркгими или расширения её функций. В результате получим:
{line-numbers: false}
```
(B && O1 || ! F1) && (C && O2 || ! F2)
```

Если вернёмся обратно к реальному коду Bash, наша команда станет выглядеть так:
{line-numbers: false}
```
(bsdtar -cjf ~/photo.tar.bz2 ~/photo && echo "bsdtar - OK" > results.txt || ! echo "bsdtar - FAILS" > results.txt) && (cp -f ~/photo.tar.bz2 /d && echo "cp - OK" >> results.txt || ! echo "cp - FAILS" >> results.txt)
```
...
На самом деле причины ошибочных решений могут быть разные. В нашем случае это нежелание учиться новому. Мы освоились с работой Bash в режиме командного интерпретатора. Но применяя полученные знания для новой задачи резервного копирования, мы не учли в полной мере её требования. Они звучат следующим образом:

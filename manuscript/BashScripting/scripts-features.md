...
В Bash есть ещё одна переменная `$*`, которая очень похожа на `$@`. Их отличие в том, что строка в `$*` интерпретируется как одно слово. Если вы используете переменную `$@`, она интерпретируется как набор слов. Предположим, наш скрипты вызывается так:
{line-numbers: false, format: Bash}
```

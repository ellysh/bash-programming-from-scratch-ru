...
Есть более изящное решение чем явный вызов команды `exit`. Если вам нужно завершить скрипт после первой команды, вернувшей код возврата отличный от нуля, можно воспользоваться вызовом `set`. Добавим его в наш скрипт. Результат приведён в листинге 3-4.

{caption: "Листинг 3-4. Добавление вызова `set`", line-numbers: true, format: Bash}
![`photo-backup-set.sh`](code/BashScripting/photo-backup-set.sh)
...
Перепишем наш скрипт так, чтобы путь для каталога фотографий читался из первого параметра. Результат приведён в листинге 3-6.
...
Обратите внимание, что при обращении к переменной `$1`, содержащей первый параметр, мы используем двойные кавычки. Если их не поставить, то скрипт не сможет корректно обработать пути, содержащие пробелы.
...
I> В нашем примере мы выполняем резервное копирование в два этапа: архивирование и переименование. Такое разделение действий нужно больше для образовательных целей. На самом деле вы можете выполнить резервное копирование одной командой `bsdtar`. Для этого просто укажите путь до архива на диске D. Например так:
{line-numbers: false, format: Bash}
```
bsdtar -cjf /d/"$1".tar.bz2 "$1" && echo "bsdtar - OK" > results.txt || ! echo "bsdtar - FAILS" > results.txt
```
...
Как мы уже знаем, есть следующие способы интеграции приложения с Bash:

1. Добавить путь до него в переменную `PATH`. Для этого отредактируйте файл `~/.bash_profile`.

2. Определить `alias` с абсолютным путём до приложения. Такое определение можно сделать в файле `~/.bashrc`.
...
Теперь скрипт запускается по имени следующим образом:
{line-numbers: false, format: Bash}
```
...
Рассмотрим пример. Предположим, что вам нужно сделать резервную копию всех PDF документов из каталога `~/Documents`. Эти документы мы можем найти следующей командой `find`:
{line-numbers: false, format: Bash}
```
...
В результате выполнения этой команды в корневой каталог диска D (`/d/`) будут скопированы архивы с каждым из найденных PDF файлов. Это неудобно. Было бы намного лучше собрать все найденные документы в один архив.
...
В результате этой команды будет скопирован только последний из найденных PDF документов. Почему это происходит?

Рассмотрим, как выглядит вызов утилиты `bsdtar` в нашем скрипте. Для простоты опустим вывод `echo`:
{line-numbers: false, format: Bash}
```
bsdtar -cjf "$1".tar.bz2 "$1"
```

Проблема заключается в том, что мы обрабатываем только первый параметр, переданный на вход скрипта. Он сохраняется в переменной с именем `1`. При этом игнорируются все дальнейшие параметры в переменных `2`, `3` и т.д. Но именно в них сохраняются результаты поиска утилиты `find`.

Чтобы исправить эту ошибку, воспользуемся переменной с именем `@`. В ней интерпретатор сохраняется все параметры, переданные в скрипт. В этом случае нам придётся немного изменить вызов `bsdtar`.

// TODO: Статическое имя архива
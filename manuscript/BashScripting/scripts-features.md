...
Поведение нового скрипта отличается. Теперь команды не связаны коннектором `&&`. Из-за этого утилита `cp` будет вызываться независимо от результата работы `bsdtar`.

// TODO: Завершение исполнения скрипта по exit. Есть ли у этой команды аргументы?
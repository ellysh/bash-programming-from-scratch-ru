...
Есть более изящное решение чем явный вызов команды `exit`. Если вам нужно завершить скрипт после первой команды, вернувшей код возврата отличный от нуля, можно воспользоватсья вызовом `set`. Добавим его в наш скрипт. Результат приведён в листинге 3-4.

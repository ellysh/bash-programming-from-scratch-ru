...
I> Для удаления объявленного ранее `alias` используйте встроенную команду интерпретатора `unalias`. Для нашего примера её вызов будет выглядеть следующим образом:
{line-numbers: false, format: Bash}
```

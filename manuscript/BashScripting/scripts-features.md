...
Для проверки корректности добавим вывод результатов каждой операции в лог-файл. Мы уже знаем, как это сделать для команд по отдельности с помощью вызова `echo`, операторов `&&` и `||`:
...
Рассмотрим, что произойдёт если `B` ложно. В этом случае отработает `F1` и вернёт ложь, благодаря отрицанию `!`. В результате всё выражение `(B && O1 || ! F1)` будет ложным. Тогда благодаря короткому замыканию, оператор `&&` не станет вычислять свой правый операнд, равный выражению `(C && O2 || F2)`. Мы получим именно то поведение, которое нам нужно.

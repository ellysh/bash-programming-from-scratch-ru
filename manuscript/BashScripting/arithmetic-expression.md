...
Предположим, что необходимо сохранить результат сложения двух чисел в переменной. Объявим её с целочисленным атрибутом `-i`. Затем сразу присвоим ей значение. Например, так:
{line-numbers: false, format: Bash}
```
declare -i var=12+7
```

В результате переменная будет равна числу 19, а не строке "12+7". Если объявить переменную с атрибутом `-i`, присваиваемое ей значение всегда будет вычисляться в математическом контексте.

Математический контекст можно объявить явно. Это делает встроенная Bash-команда let.

Предположим, что объявляется переменная без атрибута `-i`. Тогда чтобы присвоить ей значение арифметического выражения, используйте команду let. Например, так:
{line-numbers: false, format: Bash}
```
let text=5*7
```
...
Команда let принимает на вход параметры. Каждый из них должен быть корректным арифметическим выражением. Если в выражении встречаются пробелы, оно будет разделено на части из-за word splitting. В этом случае let вычисляет каждую часть выражения по отдельности. Это может привести к ошибке.
...
Здесь в результате word splitting команда let получиn на вход три выражения: "var=12", "+" и "7". Вычисление второго из них "+" приведёт к ошибке. Плюс означает арифметическую операцию сложения. Она требует двух операндов. Но в нашем случае операндов нет.
...
У команды let есть синоним — оператор ((. Внутри него word splitting не выполняется. Поэтому выражения в операторе не требуют кавычек. Всегда используйте оператор (( вместо let. Это поможет избежать ошибки.
...
Здесь вместо команды let ставятся открывающие скобки ((. В конце надо добавить закрывающие скобки )). Эта форма оператора (( возвращает код ноль в случае успешного выполнения и единицу в случае ошибки. Вычислив выражение, Bash подставит код возврата вместо него.

Вторая форма оператора (( называется **арифметической подстановкой** ([arithmetic expansion](https://wiki.bash-hackers.org/syntax/expansion/arith)). Она выглядит следующим образом:
{line-numbers: false, format: Bash}
```
var=$((12 + 7))
```

Здесь перед оператором (( ставится знак доллара $. В этом случае Bash вычислит значение выражения. Затем это значение подставляется вместо выражения. Это отличается от поведения первой формы оператора ((, которая подставляет код возврата.
...
Внутри оператора (( имена переменных можно указывать без знака доллар $. В этом случае Bash правильно подставит их значения. Например, следующие два выражения эквивалентны:
{line-numbers: true, format: Bash}
```
a=5 b=10
result=$(($a + $b))
...
I> В Bash есть устаревшая форма арифметической подстановки — оператор "$[ ]". Никогда не используйте её. Для расчёта арифметических выражений есть GNU-утилита expr. Она нужна для совместимости со старыми скриптами, написанными на Bourne Shell. Никогда не применяйте expr при разработке новых скриптов.
...
Арифметические операции выполняются в порядке их приоритета. Операции с большим приоритетом исполняются первыми.

Таблица 3-18 демонстрирует приоритет операций.
...
Порядок выполнения можно изменить с помощью круглых скобок "( )". Их содержимое называется **подвыражением** (subexpression). Bash вычисляет значения подвыражений в первую очередь. Если подвыражений несколько, они исполняются в порядке следования.

Предположим, в вашем коде используется числовая константа. Её значение можно указать в произвольной системе счисления. Для выбора системы счисления используйте префикс. Список допустимых префиксов приводится в таблице 3-19.
...
Bash всегда переводит значения чисел в десятичную систему при выводе их на экран или в файл. Встроенная команда printf меняет формат вывода чисел. Например, следующая команда выведет число в шестнадцатеричной системе:
{line-numbers: false, format: Bash}
```
printf "%x\n" 250
```

...
Рассмотрим пример. Вычислим побитовое И для чисел 5 и 3. В двоичном виде эти числа выглядят так:
{line-numbers: false}
```
5 = 101
3 = 11
...
У числа 3 оказалось меньше битов, чем у 5. Поэтому дополним двоичное представление 3 одним нулём слева. Получим:
{line-numbers: false}
```
3 = 011
```
...
В Bash операция побитового И обозначается знаком амперсанд &. Выведем на экран результат нашего вычисления:
{line-numbers: false, format: Bash}
```
echo $((5 & 3))
```
...
Число 6 надо дополнть нулём до четырёх битов:
{line-numbers: false}
```
6 = 0110
```
...
Операция побитового исключающего ИЛИ (XOR) похожа на побитовое ИЛИ. В ней над каждой парой битов операндов выполняется **логическое исключающее ИЛИ**. В исключающем ИЛИ если оба операнда равны единице, результат будет ноль. В остальных случаях результат будет такой же как и у обычного ИЛИ.

...
Операции инкремента и декремента впервые появились в [языке программирования B](https://ru.wikipedia.org/wiki/Би_(язык_программирования)), разработанном в 1969 году Кеном Томпсоном и Денисом Ритчи в Bell Labs. Позднее они перекочевали в язык C, а затем и в Bash.

Перед тем как рассматривать инкремент и декремент, познакомимся с операциями присваивания. Обычное присваивание выглядит так:
{line-numbers: false, format: Bash}
```
((var = 5))
```
...
Bash позволяет совместить присваивание с арифметическим действием или битовой операцией. Например, совмещение сложения и присваивания выглядит следующим образом:
{line-numbers: false, format: Bash}
```
((var += 5))
```
...
Теперь рассмотрим операции инкремента и декремент. У них есть две формы: постфиксная и префиксная. Они записываются по-разному. В первой знаки `++` или `--` идут после имени переменной, а во второй — до.

Рассмотрим постфиксный инкремент:
{line-numbers: false, format: Bash}
```
((var++))
```

Это выражение полностью эквивалентно следующей операции присваивания:
{line-numbers: false, format: Bash}
```
((var+=1))
```

Другими словами инкремент — это увеличение значения переменной на единицу. Аналогично декремент — это уменьшение на единицу.

Может быть непонятно, зачем нужно было вводить отдельную операцию для прибавления и вычитания единицы. Ведь уже есть достаточно компактные операции сложения и вычитания, совмещённые с присваиванием. Наиболее вероятная причина такого решения заключается в том, что в программировании очень часто используются [**счётчики цикла**](https://ru.wikipedia.org/wiki/Счётчик_цикла). Благодаря инкременту и декременту с ними становится очень удобно работать.

В чём отличие постфиксного и префиксного инкремента?

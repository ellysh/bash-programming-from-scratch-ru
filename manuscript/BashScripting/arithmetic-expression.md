...
Всего есть три типа битовых сдвигов:

1. Логический
2. Арифметический
3. Циклический

Самый простой из них — это логический. С него мы начнём знакомство с битовыми сдвигами.

Операция свдига принимает два операнда. Первый из них — это число для сдвига, а второй — количество битов, на которое происходит сдвиг.

Для выполнения логического сдвига исходное число необходимо представить в двоичном виде. Если сдвиг происходит вправо на N битов, то крайние справа N битов числа отбрасываются. Вместо них слева записываются нули. Аналогично происходит сдвиг влево: отбрасывание N битов слева и запись вместо них нудей справа.

Рассмотрим пример. Выполним логический сдвин беззнакового однобайтового целого 58 вправо на три бита. Сначала представим число в двоичном виде:
{line-numbers: false}
```
58 = 0011 1010
```

Операция сдвига влева обозначается как `<<`, а в право так `>>`. Выполним логический сдвиг числа 58 вправо на три бита:
{line-numbers: false}
```
0011 1010 >> 3 = 0011 1 = 0000 0111 = 7
```

Мы выполнили операцию сдвига в два этапа. Сначала отбросили три крайних справа бита числа и получили `0011 1`. Затем добавили три нуля слева, что дало конечный результат `0000 0111`.
...
Арифметический сдвиг влево выполняется точно так же как и логический. Для сдвига вправо необходимо отбросить необходимое количество битов справа. После этого надо дополнить результат слева тем же значением, которое имеет старший бит. Таким образом после операции знак числа не меняется.
...
Интерпретатор Bash выполняет арифметические сдвиги. Рассмотренные нами примеры можно вычислить с помощью следующих команд:
{line-numbers: true, format: Bash}
```
$ echo $((58 >> 3))
7
...
Результат сдвига 58 влево на три бита отличается, потмоу что Bash оперирует восьмибайтовыми целыми.

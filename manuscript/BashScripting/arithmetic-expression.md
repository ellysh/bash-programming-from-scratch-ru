...
Вычисление остатка в Bash обозначается знаком `%`. Обратите внимание, что в некоторых языках этим же символом обозначается операция [**modulo**](https://en.wikipedia.org/wiki/Modulo_operation). Между этими действиями есть [тонкое различие](https://habr.com/ru/post/421071). Они дают разный результат, когда знаки делимого и делителя различаются. Другими словами одно из этих чисел отрицательное, а другое положительное. Рассмотрим пример для делимого 19 и делителя -12:
{line-numbers: false}
```
19 % -12 = 7
19 modulo -12 = -5
...
При этом остаток от деления и modulo совпадают, когда знаки делимого и делителя совпадают:
{line-numbers: false}
```
19 % 12 = 19 - 12 * 1 = 7
...
Для вычисления остатка от деления `r` неполное частное `q` выбирается согласно следующим требованиям:

1. `|r| < |b|`.
2. `|r|` минимально возможное.
3. Знаки `r` и `a` совпадают.

Для вычисления modulo требования следующие:

1. `|r| < |b|`.
2. `|r|` минимально возможное.
3. Знаки `r` и `a` различаются.

Всегда помните о различии остатка от деления и modulo при программировании на любом языке.
...
{caption: "Таблица 3-12. Представление целых со знаком в дополнительном коде", width: "100%"}

...
Если ограничиться чистой математикой, побитовое отрицание — это очень простая операция. В программировании же с ним возникают сложности. Прежде всего встаёт вопрос: сколько байтов отводится под число? Предположим, что в нашем примере число 5 хранится в двухбайтовой переменной. Тогда в памяти в двоичном виде оно выглядит так:
{line-numbers: false}
```
00000000 00000101
```
...
Разные команды и операторы Bash представляют целые по-разному. Например, echo всегда выводит числа как знаковые. Команда printf позволяет указать формат выводимых целых: знаковые или беззнаковые. 

В языке Bash нет типов. Поэтому все скалярные переменные хранятся в виде строк. Поэтому интерпретация целых чисел происходит в момент их подстановки в арифметические выражения. В зависимости от контекста они могут быть подставлены как знаковые и как беззнаковые.
...
Вернёмся к примеру с побитовым отрицанием числа 5. Его результатом будет 64 битное число 0xFFFFFFFFFFFFFFFA в шестнадцатеричной системе. Его можно вывести как положительное и как отрицательное целое:
{line-numbers: true, format: Bash}
```
$ echo $((~5))
-6

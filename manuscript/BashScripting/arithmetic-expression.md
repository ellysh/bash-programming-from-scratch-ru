...
I> Для выполнения арифметики с плавающей точкой в Bash вы можете использовать калькулятор [bc](https://ru.wikipedia.org/wiki/Bc) или [dc](https://ru.wikipedia.org/wiki/Dc).
...
{caption: "Упражнение 3-6. Арифметические действия в дополнительном коде", format: text, line-numbers: false}
```
...
{caption: "Упражнение 3-7. Вычисление остатка от деления и modulo", format: text, line-numbers: false}
```
...
{caption: "Упражнение 3-8. Вычисление побитового отрицания", format: text, line-numbers: false}
```
...
{caption: "Упражнение 3-9. Вычисление побитовых И, ИЛИ, исключающего ИЛИ", format: text, line-numbers: false}
```
...
{caption: "Упражнение 3-10. Вычисление битовых сдвигов", format: text, line-numbers: false}
```
...
Как мы уже отметили, оператор `[[` неудобен для сравнения целых чисел в конструкции `if`. В этом операторе используются двухбуквенные обозначения для отношений между операндами (больше, меньше, равно, не равно и т.д.). Вместо того чтобы запоминать эти обозначения, можно просто использовать оператор `((` в форме арифметической оценки.

Рассмотрим простой пример. Предположим, что вам надо сравнить значение переменной с констнтой 5. В этом случае можно использовать следующую конструкцию `if`:
{line-numbers: true, format: Bash}
```
if (($var < 5)) 
then
...
Однако предпочтительнее всегда использовать оператор `((`.

Обратите внимание на важное отличие арифметической оценки и подстановки. Согласно POSIX-стандарту, при успешном выполнении любая программа или команда должна возвращать ноль. В случае ошибки возвращается код возврата от 1 до 255. Этот код можно интерпретировать следующим образом: ноль означает истинну, а не ноль — ложь. В этом смысле результат арифметической подстановки инвертирован, а оценки нет.

Дело в том, что арифметическая оценка — это синоним команды `let`, а значит она подчиняется требованиям POSIX-стандарта, как и любая другая команда. Арифметическая подстановка выполняется в контексте другой команды. Поэтому результат её работы зависит от реализации конкретного интерпретатора. В случае Bash если условие в операторе `((` в форме подстановки истинно, будет возвращена единица. В противном случае оператор возвращает ноль. Такое поведение было унаследованно из правил вывода логических выражений языка C.

Расчсмотрим пример. Предположим, что вы пишете однострочную команду для вывода на консоль результата сравнения переменной с числом. Она будет выглядеть следующим образом:
{line-numbers: false, format: Bash}
```
...
Логические операции, как правило, используются в форме арифметической оценки оператора `((` .

...
Если переменная объявлена как строковая, вы также можете присвоить ей результат арифметического выражения с помощью встроенной команды `let`. Например, одним из следующих способов:
...
I> У команды `let` есть устаревший синоним — оператор `$[ ]`. Не используйте его. Также для расчёта арифметических выражений есть GNU утилита `expr`. Она нужна для совместимости со скриптами, написанными на Bourne Shell. Её не следует применять при разработке новых скриптов.
...
При выводе значений переменных и констант в файл или на экран интерпретатор будет переводить их в десятичную систему. Менять формат вывода можно с помощью встроенной команды `printf`.

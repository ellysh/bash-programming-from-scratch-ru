...
Целочисленная арифметика в Bash выполняет в так называемом **математическом контексте** (math context).

Предположим, что вам необходимо сохранить в переменной `var` результат сложения двух чисел. Вы можете объявить её целочисленной и присвоить ей значение при объявлении. Например так:
...
У встроенной команды `let` есть синоним — оператор `((`. Он позволяет избежать проблемы с word splitting и поэтому указанные в нём выражения не требуют кавычек. Всегда предпочитайте использовать оператор `((` вместо `let`, чтобы избежать ошибок.
...
I> Вторая форма оператора `((` со знаком `$` является частью POSIX-стандарта. Её можно использовать для написания переносимого кода. Однако, первая форма `((` без `$` доступна только в интерпретаторах Bash, ksh и zsh .
...
I> У команды `let` есть устаревший синоним — оператор `$[ ]`. Не используйте его. Также для рассчёта арифметических выражений есть GNU утилита `expr`. Она нужна для совместимости со скриптами, наисанными на Bourne Shell. Её не следует применять при разработке новых скриптов.

Таблица 3-16 демонстрирует операции, допустимые в арфиметических выражениях.
...
| `УСЛОВИЕ ? ДЕЙСТВИЕ1 : ДЕЙСТВИЕ2` | [Тернарная условная опеарция](https://ru.wikipedia.org/wiki/Тернарная_условная_операция) | `a=$(( b < c ? b : c ))` |
| `ДЕЙСТВИЕ1, ДЕЙСТВИЕ2` | Список выражений | `((a = 4 + 5, b = 16 - 7))` |
| `( ДЕЙСТВИЕ1 )` | Группирование выражений | `a=$(( (4 + 5) * 2 ))`|
...
| 15 | `&&` | Логическое И |
| 16 | `||` | Логическое ИЛИ |
| 17 | `УСЛОВИЕ ? ДЕЙСТВИЕ1 : ДЕЙСТВИЕ2` | Тернарная условная опеарция |
| 18 | `=, *=, /=, %=, +=, -=, <<=, >>=, &=, ^=, |=` | Присваивания |
| 19 | `ДЕЙСТВИЕ1, ДЕЙСТВИЕ2` | Список выражений |

Порядок выполнения может быть изененён с помощью группирования выражений.

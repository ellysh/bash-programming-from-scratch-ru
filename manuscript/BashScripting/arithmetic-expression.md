...
[**Битовые операции**](https://ru.wikipedia.org/wiki/Битовая_операция) — это ещё один тип математических действий. Эти операции активно используется в программировании. Своё название они получили потому, что выполняются над каждым битом числа по отдельности. Чтобы выполнить битовую операцию, число представляется в двоичном виде. Затем действие выполняется над каждым его битом.
...
Битовые операции широко применяются в системном программировании. Данные, полученные по сети или от периферийного устройства, приходится каждый раз переводить в нужный формат. Обратный перевод надо делать при выдаче данных в сеть или на устройство. Для таких преобразований нужны битовые операции.
...
Устройство посылает на компьютер целое беззнаковое число. В шестнадцатеричной системе оно равно 0xAABB. Чтобы на компьютере правильно прочитать это число, надо изменить его порядок байтов. Вместо 0xAABB надо получить 0xBBAA. Попробуем это сделать.

Для изменения порядка байтов сделаем следующее:

1. Прочитаем младший байт (крайний справа) числа и сдвинем его влево на восемь битов, т.е. на один байт. Это сделает следующая Bash-команда:
{line-numbers: false, format: Bash}
```
little=$(((0xAABB & 0x00FF) << 8))
```

2. Прочитаем старший байт (крайний слева) числа и сдвинем его вправо на восемь битов. Команда:
{line-numbers: false, format: Bash}
```
big=$(((0xAABB & 0xFF00) >> 8))
```
...
Другой пример применения битовых операций. Они незаменимы для вычисления масок. Нам уже знакомы маски с правами доступа к файлам в Unix-окружении. Предположим, что некоторый файл имеет права `-rw-r--r--`. В двоичном виде эта маска представляется как двоичное число 0000 0110 0100 0100. Проверим, имеет ли владелец файла право на его исполнение. Для этого вычислим побитовое И с маской 0000 0001 0000 0000. Получим:
{line-numbers: false}
```
...
Нумерация битов в числе начинается нуля и обычно идёт справа налево.

Мы выполнили побитовое ИЛИ маски с числом 0000 0001 0000 0000. В нём восьмой бит равен единице. С его помощью мы меняем восьмой бит маски. При этом значение бита маски не важно. Он будет выставлен в единицу не зависимо от текущего значения. Все биты числа кроме восьмого равны нулям. Благодаря этому, биты маски в тех же позиция не изменятся.
...
Операционная система выполняет операции с масками каждый раз, когда вы обращаетесь к файлу. Так она проверяет ваши права на доступ к файлу.

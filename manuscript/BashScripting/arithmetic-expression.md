...
Для вычисления арифметических выражений используется оператор `((`. Результат его работы зависит от наличия знака `$`. Если перед оператором стоит `$`, арфиметическое выражение будет вычисленно и интерпретатор подставит его результат. Если знака `$` нет, после вычисления будет подставлено логическое значение, соответствующее результату. Рассмотрим эти случаи на примерах.
...
Если переменная объявлена как строковая, вы также можете присвоить ей результат арфиметического выражения с помощью встроенной команды `let`. Например, одним из следующих способов:
...
I> Кроме встроенной команды `let` есть две другие устаревшие формы для вычисления арифметических выражений: `$[ ]` и `expr`. Никогда не используйте их в Bash.

У встроенной команды `let` есть синоним — оператор `((`. Он позволяет избежать проблемы с word splitting и поэтому указанные в нём выражения не требуют кавычек. Всегда предпочитайте использовать оператор `((` вместо `let`, чтобы избежать ошибок. Кроме того, он является частью POSIX-стандарта и безопасен для написания переносимого кода.

I> Отношения оператора `((` и команды `let` напоминают `test` и `[[`. В обоих случаях лучше использовать операторы, а не команды.

Оператор `((` имеет две формы. Первая форма является полным аналогом команды `let` и выглядит следующим образом:
{line-numbers: false, format: Bash}
```
((var = 12 + 7))
```

В этом случае команда `let` была заменена на открывающие кавычки `((`, а в конце конструкции были добавлены закрывающие `))`. Как и любая команда, такая форма оператора возвращает 0 в случае успешного выполнения и 1 в случае ошибки.

Вторая форма оператора `((` выглядит следующим образом:
{line-numbers: false, format: Bash}
```
var=$((12 + 7))
```

В ней перед оператором ставится знак доллара `$`. В такой форме возвращается результат расчёта арифметического выражения, а не код возврата.
...
Предпочтительнее опускать знак `$` перед именами переменных в арифметических выражениях. Это делает код удобнее для чтения.
...
| | **Другие операции** | |
|  | | |
| `num++` | Постфикс-инкремент | |
| `num--` | Постфикс-декремент | |
| `++num` | Префикс-инкремент | |
| `--num` | Префикс-декремент | |
| `+num` | Унарный плюс | |
| `-num` | Унарный минус | |
| `УСЛОВИЕ ? ДЕЙСТВИЕ1 : ДЕЙСТВИЕ2` | [Тернарная условная опеарция](https://ru.wikipedia.org/wiki/Тернарная_условная_операция) | |
| `ДЕЙСТВИЕ1, ДЕЙСТВИЕ2` | Список выражений | |
| `( ДЕЙСТВИЕ1 )` | Группирование выражений | |
...
Не зависимо от наличия знака под целые в Bash отводится 64 бита. Это значит, что максимальное положительное знаковое число в Bash равно 7FFFFFFFFFFFFFFF в HEX или 9223372036854775807 в DEC. Минимальное отрицательное число равно 8000000000000000 в HEX или -9223372036854775808 в DEC. Максимальное беззнаковое целое равно FFFFFFFFFFFFFFFF в HEX или 18446744073709551615 в DEC.

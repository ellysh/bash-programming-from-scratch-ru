...
Возведение в степень принято записывать в виде a<sup>b</sup>, где `a` является основанием, а `b` — показателем. Например, два в степени семь записывается как 2<sup>7</sup>. В Bash это арифметическое действие обозначается двумя символа звёздочки `*`:
{line-numbers: false}
```
2**7
```
...
От результата следует отбросить дробную часть. Неполное частное для modulo считается так же, но только когда `a` и `b` имеют одинаковые знаки. Если знаки разные, формула выглядит так:
{line-numbers: false}
```
a / b = q - 1
```

От результата также надо отбросить дробную часть.

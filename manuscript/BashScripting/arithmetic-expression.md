...
Теперь возникает другой вопрос: как интерпретировать результат? Если переменная была объявлена как беззнаковое цело, нашим результатом будет число 65530. Если же переменная знаковая, её значение необходимо представить в дополнительном коде. В этом случае мы получим -6.

// TODO: Проверить утверждение. Как перевести в hex?

Все целые переменные в Bash знаковые.

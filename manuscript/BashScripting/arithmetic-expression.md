...
У прямого кода есть два недостатка. Они привели к техническим проблемам при применении кода в компьютерах. Это заставило инженеров искать альтернативное представление чисел в памяти. Так появился обратный код.
...
Для примера нам надо сложить 10 и -5. Представим эти числа в прямом коде. Предположим, что на каждое из них отводится один байт в памяти компьютера. Тогда получим следующий результат:
{line-numbers: false}
```
10 = 0000 1010
-5 = 1000 0101
...
Результат неправильный. Это означает, что сумматор не может сложить целые, представленные в прямом коде. Проблема в том, что он не учитывает старший бит числа.
...
Развитие компьютерной техники пошло по второму пути.
...
Что дало инвертирование битов значения для отрицательных чисел? Представим числа 10 и -5 в обратном коде. Затем сложим их с помощью сумматора.

Числа в памяти выглядят так:
{line-numbers: false}
```
10 = 0000 1010
-5 = 1111 1010
...
Отброшенная единица влияет на конечный результат. Нужен второй этап вычисления, чтобы её учесть. Просто добавим единицу к результату:
{line-numbers: false}
```
...
У прямого кода есть вторая проблема: представления нуля двумя способами. Её обратный код решить не смог.

...
Операция побитового исключающего ИЛИ похожа на побитовое ИЛИ. Над каждой парой битов операндов выполняется логическое исключающее ИЛИ. В нём в отличие от обычного ИЛИ, если оба разряда равны единице, результат будет ноль. В остальных случаях результат будет такой же как и у обычного ИЛИ.

Рассмотрим пример исключающего ИЛИ для чисел 14 и 5. Переведём числа в двоичный вид:
{line-numbers: false}
```
14 = 1110
5 = 101
...
В Bash эта операция будет выглядеть так:
{line-numbers: false, format: Bash}
```
echo $((14 ^ 5))
```

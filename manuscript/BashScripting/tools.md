...
Перезапустите терминал MSYS2. Теперь вы можете запускать Notepad++ по следующей команде:
{line-numbers: false}
```
notepad++.exe
```

Вместо редактирования переменной `PATH` вы можете воспользоваться командой `alias`. Она позволяет давать альтернативные имена командам. В нашем случае мы можем назвать строкой `notepad++` следующий вызов:
{line-numbers: false}
```
"/c/Program Files (x86)/Notepad++/notepad++.exe"
```
...
Теперь редактор будет запускать по команде `notepad++`. Единственная проблема этого решения заключается в том, что `alias` придётся назначать каждый раз после перезапуска терминала MSYS2. Чтобы этого не делать, можно добавить эту команду в файл `~/.bashrc`. Bash выполняет его каждый раз перед запуском терминала.

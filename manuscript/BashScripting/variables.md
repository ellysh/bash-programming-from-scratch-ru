...
Предположим, что вы объявляете переменную с именем `number` и указываете её тип — целое число размером два байта. В результате на эту переменную в памяти будет отведено ровно два байта. Далее вы присваиваете переменной значение 203 или 0xCB в шестнадцатеричном виде. В памяти она будет храниться в следующем виде:
{line-numbers: false, format: Bash}
```
00 CB
```

Обратите внимание, что для хранения значения 203 было бы достаточно одного байта. Но при объявлении переменной было заерезервированно два. Поэтому второй байт, который в нашем случае равен нулю, не будет использоваться никакими другими переменными на протяжении всей работы программы.
...
Старшая единица была отброшена. Теперь если вы прочитаете значение переменной `number`, то получите 42445 или 0xA5CD. Другими словами, записанное в `number` число 107981 потеряно и больше не может быть восстановлено. Эта проблема называется [**переполнением**](https://ru.wikipedia.org/wiki/Целочисленное_переполнение).

Рассмотрим другой пример статической типизации. Предположим, что вам нужно сохранить имя пользователя в переменной `username`. Для этого вы объявляете переменную строкового типа. Во многих языках программирования необходимо сразу указать максимально допустимую длину строки. Предположим, вы указали длину в десять символов. Затем вы присваиваете переменной имя: "Alice" в ASCII-кодировке. В этом случае в случае языка C++ строка будет сохранена в памяти так:
{line-numbers: false, format: Bash}
```
41 6C 69 63 65 00 00 00 00 00
```

I> Вы можете проверить ASCII-коды в шестнадцатеричной системе по [таблице](https://ru.wikipedia.org/wiki/ASCII).
...
[**Динамическая типизация**](https://ru.wikipedia.org/wiki/Динамическая_типизация) является альтернативным подходом. В нём способ хранения переменной определяется не в момент её объявления, а в момент присваивания нового значения. Как правило, при присваивании переменной назначается метаинформация о её текущем типе. В ходе исполнения программы переменной может быть присвоено другое значение и её тип изменится. Таким образом представление переменной в памяти также будет меняться. Динамическая типизация обычно применяется в интерпретируемых языках программирования (например, Python).

В языке Bash все переменные являются строками. Что это значит? Предположим, что вы объявили переменную равную числу 42 так:
{line-numbers: false, format: Bash}
```
number=42
```

В памяти она будет сохранена как строка следующим образом:
{line-numbers: false, format: Bash}
```
34 32 00
```

Обратите внимание, что в строго типизированном языке для хранения этой переменной было бы достаточно одного байта. Но в Bash нам потребовалось три: по байту для каждого числа плюс нуль-терминатор на конце.

#### Атрибуты

В Bash вы не можете задать тип переменной. Однако, вы можете указать для неё дополнительные атрибуты.

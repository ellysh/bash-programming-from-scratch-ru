...
Bash ограничивает допустимые значения целочисленных переменных. Если вы попробуете присвоить ей, например, строковое значение, переменная будет равна нулю. Например:
{line-numbers: false, format: Bash}
```
declare -i sum="test"
```

В результате этого объявления `sum` будет равна нулю.

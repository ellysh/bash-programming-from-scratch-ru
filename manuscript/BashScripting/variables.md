...
Зачем интерпретатору понадобились переменные в глобальной области видимости, т.е. доступные каждой запускаемой программе? В Unix есть набор настроек и переменных, которые должны передаваться во все запускаемые пользователем приложения. Например, региональные настройки. Учитывая их, запущенное приложение может адаптировать свой интерфейс. Именно для этих целей используются переменные окружения.

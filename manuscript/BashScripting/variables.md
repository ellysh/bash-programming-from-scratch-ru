...
Интерпретатор Bash может работать в двух основных режимах: интерактивном (командная оболочка) и неинтерактивном (исполнение скриптов). В каждом из них переменные решают сходные задачи, но контексты их использования различаются. Из-за этого в Bash больше признаков для классификации переменных, чем в других языках.
...
Если вы не использовали служебное слово `export` при объявлении переменной и не передавали её при вызове приложения, то она будет локальной. Это означает, что переменная будет доступна только в текущем экземпляре интерпретатора. Никакие дочерние процессы (кроме subshell) не наследуют локальные переменные.
...
Однако, если вы попытаетесь получить доступ к переменной из дочернего процесса, то получите пустое значение. Чтобы это сделать, создайте скрипт с выводом `echo` или запустите новый экземпляр интерпретатора в окне терминала. Например, так:
{line-numbers: false, format: Bash}
```
...
Тогда значение переменной `filename` корректно напечатается из дочернего процесса `bash`. Но причина этого не в том, что переменная была передана в дочерний процесс, а в том что её значение попало в параметр командной строки вызова `bash`.

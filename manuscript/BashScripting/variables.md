...
Вы можете вызвать `declare` с опцией `-p`. Тогда команда выведет список всех объявленных переменных с их атрибутами. Можно указать имя переменной, чтобы получить информацию только по ней. Например так:
{line-numbers: false, format: Bash}
```
declare -p PATH
```

Все атрибуты, устанавливаемые командой `declare`, приведены в таблице 3-7.
...
|  | |
| `-A` | Объявленная переменная является ассоциативным массивом. В этом случае кажому элементу массива соответствует ключ-строка. |
|  | |
| `-f` | Опция используется для вывода объявленных [**функций**](https://ru.wikipedia.org/wiki/Функция_(программирование)). Можно указать имя функции для вывода информации только по ней. |

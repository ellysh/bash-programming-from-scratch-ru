...
Интерпретатор Bash может работать в двух основных режимах: интерактивном (командная оболочка) и неинтерактивном (исполнение скриптов). Переменные активно используются в каждом из них. Различие решаемых в этих режимах задач привело к появлению нескольких признаков для классификации переменных в Bash.
...
Все зарезервированные переменные Bash находятся в глобальной **области видимости**, то есть являются переменными окружения. Что это означает?

В Unix есть набор настроек и переменных, которые должны передаваться во все запускаемые пользователем приложения. Например, региональные настройки. Учитывая их, запущенное приложение может адаптировать свой интерфейс.

Когда один процесс порождает другой, дочерний процесс наследует все переменные окружения от родительского. В данном случае наследование означает создание собственной копии. Таким образом все утилиты и приложения, запущенные из командного интерпретатора, по умолчанию наследуют его переменные окружения. Это позволяет решить проблему передачи параметров во все приложения, запускаемые пользователем.

Важно заметить, что дочерние процессы могут изменять свои переменные окружения. Это никак не отразится на соответствующих переменных родительского процесса. Однако, порождённые ими процессы будут наследовать уже изменённые переменные.
...
Так же вы можете поместить переменную в окружение после её объявления:
{line-numbers: true, format: Bash}
```
...
Вы можете добавить или переопределить переменные окружение для одного конкретного запускаемого приложения. Если вы работаете в Bash, просто перечислите их имена и значения, разделённые пробелом, перед его вызовом. Например, следующим образом:
{line-numbers: false, format: Bash}
```
...
Если вызвать утилиту `env` без параметров, она выведет список всех определённых в данный момент переменных окружения. Попробуйте выполнить следующую команду в своём терминале:
{line-numbers: false, format: Bash}
```
env
```

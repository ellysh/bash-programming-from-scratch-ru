...
|  | | | |
| 4 | Command Substitution | Подстановка вместо команды её вывода. | `echo $(cat README.txt)` |
|  | | | |
| 5 | Arithmetic Expansion | Подстановка вместо арифметических выражений их значений. | `echo $((4+3))` |
|  | | | |
...
I> Обратите внимание на одинарные кавычки `'`. Они отключают все подстановки для заключённой в них строки. Двойные кавычки отключают некоторые подстановки, но подстановки команд и параметров по прежнему работают. Если в нашем примере использовать двойные кавычки, то в результате подстановки получится следующая команда вызова `bash`:
{line-numbers: false, format: Bash}
```

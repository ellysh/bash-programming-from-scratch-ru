...
Вы можете вызвать `declare` с опцией `-p`. Тогда помимо имён и значений переменных будут выведены их атрибуты. Можно ограничить вывод команды `declare` только интересующей вас переменной. Для этого передайте её имя в качестве параметра команды следующим образом:
{line-numbers: false, format: Bash}
```
declare -p PATH
```
...
Мы рассмотрели опции команды `declare` для получения информации об уже объявленных переменных и функциях. Теперь рассмотрим, как с её помощью объявлять переменные и устанавливать атрибуты. Наиболее часто используемые из них приведены в таблице 3-7.
...
В результате первая переменная, объявленная как целочисленная, будет иметь значение 13, то есть суммы чисел 11 и 2. Вторая будет равна тексту "11+2". Обратите внимание, что и `sum`, и `text` хранятся в памяти в виде строк. Таким образом атрибут `-i` не задаёт тип переменной, а ограничивает её допустимые значения. Если вы попробуете присвоить `sum` строковое значение, она станет равна нулю. Например в результате следующего объявления:
{line-numbers: false, format: Bash}
```
declare -i sum="test"
```

Кроме того атрибут `-i` позволяет выполнять арифметические операции над переменной без подстановки арифметических выражений. Например:
{line-numbers: true, format: Bash}
```
sum=sum+1       # 13 + 1 = 14
sum+=1          # 14 + 1 = 15
...
Вместо арифметических операций над числами произошло добавление суффикса к строке.

Атрибут `-r` позволяет объявлять переменную константой. Например так:
{line-numbers: false, format: Bash}
```
declare -r filename="README.txt"
```

После такого объявления Bash вернёт сообщение об ошибке при попытке переобъявить или удалить переменную с именем `filename`. То есть следующие команды окажутся невозможны:
{line-numbers: true, format: Bash}
```
filename="123.txt"
unset filename
...
Команда `declare`, вызванная с атрибутом `-x`, объявляет переменную окружения. То же самое делает команда `export`. Это значит, что следующие два

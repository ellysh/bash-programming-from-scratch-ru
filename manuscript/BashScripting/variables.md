...
Все зарезервированные переменные Bash находятся в **глобальной области видимости** (global scope). Переменные в этой области видимости называются **переменными окружения** (environment variables). Получается, что все зарезервированные переменные являются переменными окружения. Пользовательские переменные также можно объявлять в глобальной области видимости. То есть они тоже могут быть переменными окружения.

Зачем интерпретатор хранит переменные в глобальной области видимости? Дело в том, что в Unix есть специальный набор настроек и переменных. Они влияют на поведение каждого приложения, запускаемого пользователем. Например, региональные настройки. Зная их, каждое запущенное приложение может адаптировать свой интерфейс. Именно такие настройки передаются через переменные окружения.

Предположим, что какой-то процесс порождает дочерний процесс. В этом случае дочерний процесс копирует все переменные окружения родителя. Таким образом все утилиты и приложения, запущенные из командного интерпретатора, наследуют его переменные окружения. Так глобальные настройки передаются во все запускаемые пользователем приложения.
...
Также вы можете поместить переменную в глобальную область видимости после её объявления:
{line-numbers: true, format: Bash}
```

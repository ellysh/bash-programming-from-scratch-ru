...
Атрибуты переменной указываются с помощью встроенной команды declare. При вызове без параметров она выводит все объявленные переменные: локальные и окружения. Тот же вывод можно получить командой set.

Вы можете вызвать команду declare с опцией `-p`. Тогда в вывод попадут не только имена и значения переменных, но и их атрибуты. Можно ограничить вывод команды declare только интересующей вас переменной. Для этого передайте её имя в качестве параметра следующим образом:
{line-numbers: false, format: Bash}
```
declare -p PATH
```

Команда declare выводит информацию не только об объявленных переменных, но и о доступных [**подпрограммах**](https://ru.wikipedia.org/wiki/Функция_(программирование)). В Bash они называются **функциями**. Подпрограмма — это фрагмент программы или самостоятельный блок кода, который выполняет определённую задачу. Например, чтобы получить информацию о функции `quote`, вызовите команду declare следующим образом:
{line-numbers: false, format: Bash}
```
declare -f quote
```

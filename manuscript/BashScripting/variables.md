...
Вернёмся к вопросу о названиях структур данных в Bash. Ответ на него знает только автор языка. Однако, мы можем предположить. Название "массив" даёт пользователю подсказку о том, как следует работать с этой структурой. Имея опыт в других языках (например, C) пользователь знает, что читать и писать отдельные элементы массива надо по индексу. Поэтому он сможет использовать уже знакомый ему синтаксис языка C в Bash. При этом пользователю совсем необязательно знать, как на самом деле "массив" Bash хранится в памяти.

#### Атрибуты

У языка Bash нет системы типов. Но у него есть переменные скалярных и составных типов. Тип переменной выбирается при её определении. Для этого надо указать метаинформацию, которая в Bash называется **атрибутами**. Кроме типа атрибуты определяют константность и область видимости переменной.

Чтобы указать атрибуты переменной, используйте встроенную команду declare. Если вызвать её без параметров, она выведет имена и значения всех объявленных в данный момент переменных: локальные и окружения. Тот же вывод можно получить командой set.
...
Команда declare без параметров выводит информацию не только об объявленных переменных, но и о доступных [**подпрограммах**](https://ru.wikipedia.org/wiki/Функция_(программирование)). В Bash они называются **функциями**. Подпрограмма — это фрагмент программы или самостоятельный блок кода, который выполняет определённую задачу.
...
После выполнения команды, на экране будет выведено определение функции.
...
Команда declare не только выводит информацию об уже объявленных переменных и функциях. С её помощью можно установить атрибуты при объявлении новой переменной.
...
|  | |
| `-r` | Объявление константы. Это означает, что после объявления этой переменной нельзя присвоить другое значение. |
|  | |
...
Обратите внимание, что обе переменные хранятся в памяти в виде строк. Это значит, что опция `-i` не задаёт тип переменной. Вместо этого опция ограничивает её допустимые значения.
...
Предположим, вы объявили переменную как целочисленную. Теперь над ней можно выполнять арифметические операции. При этом подстановка арифметических выражений не нужна. Например, следующие команды выполнятся корректно (в комментариях приведены результаты):
{line-numbers: true, format: Bash}
```
sum=sum+1       # 13 + 1 = 14
sum+=1          # 14 + 1 = 15
...
Теперь при каждой попытке изменить значение переменной `filename` или удалить её Bash выводит сообщение об ошибке. Это значит, что следующие команды завершатся с ошибкой:
{line-numbers: true, format: Bash}
```
filename="123.txt"
unset filename
...
Команда declare с опцией `-x` объявляет переменную окружения. То же самое объявление делает команда export. Следующие два

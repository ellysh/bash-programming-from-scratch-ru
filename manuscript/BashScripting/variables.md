...
В языке Bash все [**скалярные переменные**](https://en.wikipedia.org/wiki/Variable_(computer_science)) являются строками. Что это значит? Предположим, что вы объявили переменную равную числу 42 так:
{line-numbers: false, format: Bash}
```
declare -i number=42
```
...
В Bash вы не можете задать тип переменной. Однако, вы можете указать для неё дополнительные атрибуты. Эти атрибуты...

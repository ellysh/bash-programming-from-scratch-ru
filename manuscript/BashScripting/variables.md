...
В Bash версии 4 добавили ассоциативные массивы. В них индексы — это не числа, а строки. Такая строка-индекс называется **ключом** (key). Ассоциативный массив по указанной строке-индексу возвращают соответствующую ей строку-значение. Когда это может быть полезно? Рассмотрим пример.

Предположим, нам нужен скрипт для хранения контактов. Скрипт позволяет добавлять в список имя человека, его email и номер телефона. Для простоты можно опустить фамилию и хранить только имя. По запросу скрипт должен выводить контактные данные человека. Какую структуру данных выбрать?

Если применить индексируемый массив, поиск по списку будет долгим и неэффективным. Придётся перебирать все элементы массива друг за другом и сравнивать имя человека с искомым. Если нужное имя найдено, выводить на экран соответствующие контактные данные.

Ассоциативный массив сделает поиск по контактам быстрее и проще. В этом случае перебирать все элементы массива не нужно. Достаточно указать ключ и получить соответствующий элемент массива. Рассмотрим детали реализации этого решения.
...
Ассоциативный массив всегда объявляется с командой declare и её опцией `-A`. Bash не сможет автоматически вывести его тип, даже если указать в качестве индексов строки. Поэтому в результате следующего объявления вы получите индексируемый массив, а не ассоциативный:
{line-numbers: false, format: Bash}
```
...
Мы объявили ассоциативный массив. Его элементы доступны по ключам также. В нашем случае ключ - это имя человека.
...
Если нужно вывести список всех ключей, поставьте восклицательный знак ! перед именем массива:
{line-numbers: true, format: Bash}
```
$ echo "${!contacts[@]}"

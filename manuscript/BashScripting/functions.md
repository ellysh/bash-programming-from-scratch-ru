...
Функции в Bash, в отличие от большинства других процедурных языков, не могут возвращать произвольное значение. Вместо этого Bash предоставляет встроенные средства для возврата только кода возврата, то есть целочисленного значения от 0 до 255.
...
4. Если в теле функции нет команды `return`, то после выполнения последней команды функции, происходит переход обратно в место её вызова.

В других языках программирования ключевое слово `return` позволяет вернуть переменную любого типа: число, строку или массив. Как можно добиться аналогичного поведения в Bash?
...
Второй способ вернуть значение из функции — воспользоваться переменной, глобальной в области видимости скрипта. Такая переменная доступна в любом месте скрипта. По умолчанию все переменные, объявленные в скрипте вне тел функций являются глобальными.

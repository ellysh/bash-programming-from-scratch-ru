...
Конфликт имён — это серьёзная проблема. Она возникает в Bash, когда функции объявляют свои переменные в глобальном пространстве имён. В результате, имена двух переменных могу совпасть. В этом случае доступ к ним происходит из разных функций. Это может привести к потере данных.

Чтобы решить конфликт имён в Bash, используйте механизм ограничения области видимости переменных. Рассмотрим его.

Если объявить переменную с ключевым словом local, её [**область видимости**](https://ru.wikipedia.org/wiki/Пространство_имён_(программирование)) ограничится телом функции. Другими словами, переменная будет доступна только в теле функции.
...
Область видимости локальной переменной ограничена временем исполнения функции, в которой она объявлена. Такая область видимости называется [**динамической**](https://ru.wikipedia.org/wiki/Область_видимости#Лексические_vs._динамические_области_видимости). Она встречается редко в современных языках. Чаще в них применяется **лексическая** область видимости. В ней переменная доступна только в теле функции, но не за его пределами (например, в вызываемых функциях).

Локальные переменные не попадают в глобальную область видимости. Это гарантирует, что никакая функция их случайно не перезапишет.
...
Неосторожное обращение с локальными переменными может привести к ошибке. Дело в том, что они скрывают глобальные переменные с теми же именами. Рассмотрим пример.

Предположим, вы пишете функцию для обработки файла. Например, она с помощью утилиты grep ищет шаблон в файле. Функция может выглядеть так:
{line-numbers: true, format: Bash}
```
check_license()
{
...
Как тогда отработает функция `check_license`? Она выполнится корректно, потому что произойдёт **сокрытие глобальной переменной**. То есть при обращении к имени `filename` в теле функции Bash подставит локальную переменную. Из-за сокрытия в теле функции нельзя получить доступ к глобальной переменной `filename`.

Если сокрытие переменных произошло случайно, может возникнуть ошибка. Старайтесь исключить саму возможность такой ситуации. Для этого используйте префикс или постфикс для локальных переменных функций. Это может быть символ подчёркивания _ или любой другой.
...
В этом примере массив `files` попадёт в глобальную область видимости:
{line-numbers: true, format: Bash}
```
check_license()
{
...
Мы познакомились с функциями в Bash. Вот общие рекомендации по их применению:

...
Здесь переменная `_result_variable` объявлена как локальная. Это значит, что она будет доступна для чтения и изменения только в теле `code_to_error` и любых вызываемых ею функциях. Такое ограничение области видимости гарантирует вам, что случайной перезаписи значение одноимённой глобальной переменной не произойдёт.

...
Может возникнуть вопрос: в чём отличие функции от псевдонимов? Если мы объявим следующий псевдоним, получим то же самое поведение, какое дала нам функция `mem`:
{line-numbers: false, format: Bash}
```
alias mem="cat /proc/meminfo"
```

Прежде всего, и функции, и псевдонимы являются встроенными механизмами интерпретатора Bash. Отчасти их возможности пересекаются, когда вы работаете с Bash в режиме командного интерпретатора. Однако, принцип их работы совершенно различен.

Механизм псевдонимов представляет собой подстановку текста во введённой вами команде. Это означает, что Bash просто подставляет значение alias вместо части введённой вами команды.
...
Теперь рассмотрим, как работают функции. В отличие от псевдонимов тело функции не подставляется вместо её имени во введённую вами команду. Как только Bash встречает имя функции в команде, он вызывает её и начинает исполнять её тело. Рассмотрим пример.
...
Ключевое слово `command`, введённое перед командой, выключает подстановку псевдонимов и вызовы функций, имена которых встречаются в этой команде. С использованием `command` наша функция `cat` будет выглядеть следующим образом:
{line-numbers: false, format: Bash}
```
cat() { command cat -n "$@"; }
```
...
Функция будут удобнее псевдонимов в следующих случаях:
...
Вы не сможете параметризовать путь, используя псевдоним. Например, следующий вариант не заработает, потому что путь должен идти до опции `-type`:
{line-numbers: false, format: Bash}
```
alias="find -type f"
```

...
Bash перед выполнением команды `cat -n $@` проверяет список объявленных функций. В этом списке он найдёт функцию с именем `cat`, тело которой и выполняется в данный момент. В результате вместо вызова утилиты `cat` произойдёт вызов одноимённой функции. Затем её вызов будет повторяться снова и снова. Возникнет бесконечная рекурсия, которая похожа на бесконечный цикл.

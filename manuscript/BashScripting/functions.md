...
Ключевое слово `local` позволяет ограничить область видимости переменной телом функции. Рассмотрим наш последний вариант объявления `code_to_error`:
{line-numbers: true, format: Bash}
```
code_to_error()
{
...
Предположим, что вы пишете функцию для нахождения суммы квадратов двух чисел, введённых пользователем. Она может выглядеть например следующим образом:
{line-numbers: true, format: Bash}
```
sum_sqr()
{
...
Тогда после следующего вызова функции, в глобальную переменную `result` будет записано число 13:
{line-numbers: false, format: Bash}
```
sum_sqr 2 3
```

Теперь представьте, что вы решили передавать в функцию числа через глобальные переменные с именами `a` и `b`
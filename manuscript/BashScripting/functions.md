...
Подведём итоги нашего сравнения функций и alias. Если вам нужно просто сократить длинную команду — используйте alias, а не функции.

Функции будут удобнее alias в следующих случаях:
...
Вы не сможете параметризовать путь, используя alias. Например, следующий вариант не заработает, потому что путь должен идти до опции `-t`:
{line-numbers: false, format: Bash}
```
alias="find -type f"
```

Функция легко справляется с этой проблемой:
{line-numbers: false, format: Bash}
```

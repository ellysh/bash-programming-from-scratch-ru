...
Чтобы вернуть значение из функции, процедурные языки имеют встроенную команду. Обычно она называется return. В Bash эта команда тоже есть. Но её поведение принципиально отличается. Команда return в Bash не возвращает значение. Она передаёт код возврата, то есть целое число от 0 до 255.

Полный алгоритм вызова и выполнения функции выглядит так:

1. При выполнении команды встречается имя функции.

2. Интерпретатор переходит на тело функции и исполняет с его первой команды.
...
В других процедурных языках команда return возвращает переменную любого типа: число, строку или массив. Аналогичного поведения можно добиться и в Bash. Для этого есть три способа:
...
Рассмотрим примеры каждого из трёх способов.

Для вывода сообщений об ошибках мы написали функции `code_to_error` и `print_error`. Они выглядят так:
{line-numbers: true, format: Bash}
```
code_to_error()
{
...
Здесь применяется первый способ возврата значения. Вызов функции `code_to_error` помещается в подстановку команды. Благодаря этому, Bash подставит в место вызова функции всё, что она выведет на консоль.
...
2. Входным параметр `$2` функции `print_error` — это имя файла, доступ к которому вызвал ошибку.
...
Перепишем функции `code_to_error` и `print_error`. Запишем результат `code_to_error` в глобальную переменную. Затем прочитаем эту переменную в функции `print_error`. Получится следующее:
{line-numbers: true, format: Bash}
```
code_to_error()
{

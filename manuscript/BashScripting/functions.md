...
Конфликт имён — это серьёзная проблема. Она возникает всякий раз при сочетании глобальных переменных и функций. Чтобы решить проблему, процедурные языки предлагают механизмы ограничения области видимости переменных.

Хорошая практика программирования на процедурных языках предлагает отказаться от глобальных переменных. К сожалению, в Bash сложно следовать этому совету. При объявлении переменной в Bash она по умолчанию попадает в глобальную область видимости. С другой стороны в Bash есть механизм ограничить область видимости. Рассмотрим его.

Ключевое слово local ограничивает область видимости переменной. Она будет доступна только в теле функции. Рассмотрим наш последний вариант объявления `code_to_error`:
{line-numbers: true, format: Bash}
```
code_to_error()
{

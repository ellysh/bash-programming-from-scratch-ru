...
В рассмотренных нами примерах двойные кавычки необязательны. При подстановки переменной в выражение `[[` globbing и world splitting не выполняются. То есть интерпретатор не пытается самостоятельно обработать значение переменной, а использует его как есть. Однако, если строка с которой происходит сравнение содержит пробелы, возникнет синтаксическая ошибка. Например, в следующем случае:
{line-numbers: true, format: Bash}
```
if [[ "$var" = abc def ]]
then

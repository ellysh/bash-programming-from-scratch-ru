...
```
Нам дана команда, которая проверяет файлы в каталоге с именем target на наличие в них шаблона "123". Файлы, в которых шаблон встречается, копируются в текущий каталога. Если шаблон в файле не обнаружен, он удаляется из каталога target.

Команда выглядит следующим образом:
( grep -Rl "123" target | xargs cp -t . && echo "cp - OK" || ! echo "cp - FAILS" ) && ( grep -RL "123" target | xargs rm && echo "rm - OK" || echo "rm - FAILS" )

Замените в ней логические операторы && и || на конструкции if-else.
```

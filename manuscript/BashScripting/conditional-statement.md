...
Мы познакомились с оператором if. В качестве условия в нём вызывается встроенная команда Bash или сторонняя утилита. Например, вы можете вызвать утилиту grep и в зависимости от её результата выполнить то или иное действие.

Если вы проверяете результат grep в условии оператора if, используйте опцию утилиты `-q`. Благодаря ей, grep не станет выводит результат в стандартный поток вывода. Вместо этого grep вернёт код ноль при первом вхождении искомой строки или шаблона. Например:
{line-numbers: true, format: Bash}
```
...
Теперь предположим, что в условии if вам надо сравнить две строки или числа. Для работы со строками в Bash есть специальный оператор [[. Двойные квадратные скобки являются [**зарезервированным словом**](https://ru.wikipedia.org/wiki/Зарезервированное_слово) интерпретатора. Это значит, что интерпретатор обрабатывает его самостоятельно.

W> В Bourne shell оператора [[ нет. Если вам важна POSIX-совместимость, используйте устаревший оператор [test](http://mywiki.wooledge.org/BashFAQ/031) или его синонимом [. Но никогда не используйте test в Bash! Возможности test по сравнению с [[ ограничены, а правильные способы применения неочевидны.

Начнём с простого примера использования [[. Предположим, что надо сравнить две строки. В этом случае условие if выглядит так:
{line-numbers: true, format: Bash}
```
if [[ "abc" = "abc" ]]
then
...
Чтобы избежать подобных ошибок, всегда используйте кавычки при работе со строками. Если следовать этому правилу, прошлый пример станет такми:
{line-numbers: true, format: Bash}
```
if [[ "$var" = "abc def" ]]
then
...
В операторе [[ также можно сравнить значения двух переменных друг с другом. Например, так:
{line-numbers: true, format: Bash}
```
if [[ "$var" = "$filename" ]]
then
...
В операторе [[ можно использовать логические операции И, ИЛИ и НЕ. Они позволяют скомбинировать несколько выражений в одно условие. Таблица 3-9 приводит примеры таких условий.
...
В операторе [[ можно сравнивать не только строки. Он позволяет проверить файлы и каталоги на различные условия. Эти операции проверки приводятся в таблице 3-10.
...
Кроме строк оператор [[ может сравнивать и целые числа. Соответствующие операции приведены в таблице 3-11.
...
Таблица 3-11 вызывает вопросы. Эти операции сложнее запомнить чем привычные знаки сравнения чисел (<, > и =). Почему в операторе [[ используются именно двухбуквенные сокращения? Чтобы ответить на этот вопрос, обратимся к истории оператора [[.
...
Рассмотрим синтаксис оператора test. Когда он был сторонней утилитой, формат его входных параметров подчинялся правилам Bourne shell. Например, вот типичный вызов test для сравнения значения переменной `var` и числа:
{line-numbers: false, format: Bash}
```
test "$var" -eq 5
```
...
В Bourne shell для оператора test добавили синоним [. Единственное отличие между ними — это требование закрывающей скобки ]. Для test она не нужна. С помощью синонима перепишем условие конструкции if так:
{line-numbers: true, format: Bash}
```
if [ "$var" -eq 5 ]
then
...
В случае сравнения строк задачи обеспечить обратную совместимость не стояло. Первая версия утилиты test вообще не поддерживала знаков сравнения < и >. Такая поддержка появилась для строк, но только в расширении POSIX-стандарта. Там знаки сравнения приходилось экранировать `/<` и `/>`. Из стандарта они попали в оператор [[, но уже без экранирования.

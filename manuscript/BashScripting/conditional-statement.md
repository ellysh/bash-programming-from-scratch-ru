...
Работая с утилитой find, мы впервые познакомились с условными конструкциями. Затем мы узнали, что у Bash есть собственные логические операторы И (&&) и ИЛИ (||). Это не единственные формы ветвления в языке Bash.

М рассмотрим операторы if и case. Они часто используются в скриптах. Эти операторы взаимозаменяемы. Но каждый из них лучше справляется с определёнными задачами.

### Оператор if

Представьте, что вы пишете однострочную команду. При этом вы пытаетесь сделать её как можно компактнее. Согласитесь, что короткую команду проще набрать и шанс допустить в ней ошибку меньше.

Теперь представьте, что вы пишете скрипт. Он храниться на жёстком диске. При этом вы регулярно его вызываете и иногда изменяете. Здесь компактность уже не так важна. В первую очередь скрипт должен быть удобен для чтения и редактирования.

Операторы && и || хорошо подходят для однострочных команд. Но для скриптов есть лучшие альтернативы. На самом деле всё зависит от конкретного случая. Иногда операторы && и || отлично вписываются в код скрипта. Но в большинстве случаев их лучше заменить на оператор if или case. Рассмотрим эти случаи подробнее.

Ещё раз обратимся к скрипту для резервного копирования из листинга 3-9. Вызов утилиты bsdtar выглядит следующим образом:
...
Чтобы сделать скрипт проще для чтения, мы разбили вызовы утилит bsdtar и mv на отдельные команды. Это помогло, но лишь отчасти. Вызов bsdtar всё ещё слишком длинный. При его изменении легко допустить ошибку. Это верный признак плохого технического решения.

Распишем алгоритм вызова bsdtar по шагам:

1. Прочитать из переменной `$@` список файлов и каталогов. Архивировать и сжать их.

2. Если команда выполнилась успешно, записать в лог-файл строку "bsdtar - OK".
...
Эту конструкцию можно записать и в одну строку. Для этого перед then и fi добавьте по точке с запятой. Например:
{line-numbers: false}
```
...
В качестве УСЛОВИЯ вызывается утилита cmp. Она побайтово сравнивает содержимое двух файлов. Если они отличаются, cmp печатает в стандартный поток вывода позицию первого различающегося символа. Сейчас нас интересует только код возврата утилиты. Поэтому мы перенаправляем её вывод в файл [`/dev/null`](https://ru.wikipedia.org/wiki//dev/null). Это специальный системный файл. Запись в него всегда проходит успешно, а все записанные данные удаляются.

Итак, если содержимое файлов `file1.txt` и `file2.txt` совпадает, утилита cmp вернёт код возврата 0. Тогда команда echo выведет соответствующее сообщение на экран.

До сих пор действие совершалось в случае выполнения условия. Но бывают случаи, когда с помощью условия выбирается одно из двух действий. Именно так работает конструкция if-else. Её можно записать в следующем виде:
{line-numbers: true}
```
if УСЛОВИЕ
then
...
В этой конструкции блок команд, обозначенный как ДЕЙСТВИЕ_2, выполнится, если УСЛОВИЕ вернёт код ошибки отличный от 0. В противном случае выполнится ДЕЙСТВИЕ_1.

Конструкцию if-else можно дополнить условиями и действиями с помощью блоков elif. Например, в зависимости от значения переменной вы выбираете одно из трёх действий. Это можно сделать с помощью такой конструкции if:
выглядеть так:
{line-numbers: true}
```
if УСЛОВИЕ_1
...
Количество блоков elif не ограничено. Вы можете добавлять их столько, сколько вам нужно.

Добавим в наш пример сравнения файлов вывод сообщения об их различии. Для этого воспользуемся конструкцией if-else. Получим следующее:
{line-numbers: true, format: Bash}
```
if cmp file1.txt file2.txt &> /dev/null
then
...
Согласитесь, что теперь читать и редактировать код стало удобнее. Мы можем его ещё упростить. Применим технику [**раннего возврата**](https://habr.com/ru/post/348074/) и заменим конструкцию if-else на if:
{line-numbers: true, format: Bash}
```
if ! bsdtar -cjf "$1".tar.bz2 "$@"
then
...
Поведение нашего кода осталось таким же. С помощью логического отрицания ! мы инвертировали результат утилиты bsdtar. Теперь если она завершится с ошибкой, условие оператора if станет истинным. В этом случае выводится сообщение "bsdtar - FAILS" и вызывается команда exit. Если утилита bsdtar отработает корректно, в лог-файл выводится строка "bsdtar - OK".

Рассмотрим технику раннего возврата. Это полезный приём, который сделает ваш код проще и понятнее для чтения. Его идея в том, чтобы в случае ошибки завершать программу как можно раньше. В противном случае вам не избежать **вложенных конструкций if**.

Рассмотрим пример. Представьте, что некоторый алгоритм состоит из пяти действий. Каждое последующее действие выполняется только при успешном завершении предыдущего. Этот алгоритм можно реализовать с помощью вложенных конструкций if: 
{line-numbers: true}
```
if ДЕЙСТВИЕ_1
then
...
Такое вложение выглядит запутанным. Добавьте в него блоки else с выводами сообщений об ошибках для каждого действия. Читать такой код станет ещё сложнее.
...
В этом примере мы впервые использовали [**комментарии**](https://ru.wikipedia.org/wiki/Комментарии_(программирование)): "# обработка ошибки". Комментарий — это строка или её часть, которую игнорирует интерпретатор. В Bash комментарием является всё, что идёт после символа решётка #.

Польза комментариев — это предмет бесконечных споров в сообществе программистов. Они нужны для пояснений к коду. Однако, некоторые считают, что наличие комментариев — это признак непонятного, плохо написанного кода. Если вы только начинаете изучать программирование, обязательно используйте их. Комментируйте сложные конструкции в своих скриптах, смысл которых вы можете забыть. В будущем это поможет вам вспомнить, как они работают.

Вернёмся к нашему алгоритму из пяти действий. Конструкция if и ранний возврат позволили написать нам простой и понятный код. Предположим, что каждому действию алгоритма соответствует одна короткая команда. Все ошибки обрабатываются командой exit без вывода в лог-файл. В этом случае конструкции if можно заменить на оператор ||. При этом код останется простым и понятным. Он будет выглядеть например так:
{line-numbers: true}
```
ДЕЙСТВИЕ_1 || exit 1
ДЕЙСТВИЕ_2 || exit 1
...
В скрипте мы заменили операторы && и || на конструкцию if. Но в общем случае они не эквивалентны. Рассмотрим пример. Предположим, у нас есть выражение из трёх команд A, B и C:
{line-numbers: false}
```
A && B || C
```

Можно предположить, что следующая конструкция if-else даст такое же поведение:
{line-numbers: false}
```
if A
then
...
```
Нам дана Bash команда. Она ищет строку "123" в файлах каталога с именем target. Если в файле встречается строка, он копируются в текущий каталога. Если строки в файле нет, он удаляется из каталога target.

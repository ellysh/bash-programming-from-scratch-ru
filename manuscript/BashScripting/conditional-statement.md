...
Предположим, что вам необходимо сохранить в переменной `var` результат сложения двух чисел. Вы можете объявить её целочисленной и выполнить присвоение при её объявлении. Например так:
{line-numbers: false, format: Bash}
```
declare -i var=12+7
```

Если переменная объявлена как строковая, вы так же можете присвоить ей результат арфиметического выражения с помощью оператора `((`. Например:
{line-numbers: false, format: Bash}
```
var=$((12+7))
```

I> Оператор `((` является синонимом встроенной команды `let`. Точно так же как оператор `[[` является синонимом `test`.

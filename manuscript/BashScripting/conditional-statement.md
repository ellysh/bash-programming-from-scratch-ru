...
{caption: "Упражнение 3-3. Использование оператора if", format: text, line-numbers: false}
```
...
В таблице 3-4 приведены все операции сравнения строк, допустимые в конструкции `[[`.

{caption: "Таблица 3-4. Операции сравнения строк `[[`", width: "100%"}
...
Для построения сложных выражений в операторе `[[` вы можете воспользоваться логическими операциями, приведёнными в таблице 3-5.

{caption: "Таблица 3-5. Логические операции `[[`", width: "100%"}
| Операция | Описание | Пример |
| --- | --- | --- |
...
Кроме сравнения строк, оператор `[[` позволяет проверять файлы на различные условия. Наиболее часто используемые из них приведены в таблице 3-6.

{caption: "Таблица 3-6. Операции проверки файлов `[[`", width: "100%"}
| Операция | Описание | Пример |
| --- | --- | --- |
...
Оператор `[[` позволяет сравнивать не только строки, но и целые числа. Таблица 3-7 демонстрирует соответствующие операции.

{caption: "Таблица 3-7. Операции сравнения целых чисел `[[`", width: "100%"}
| Операция | Описание | Пример |
| --- | --- | --- |
...
Таблица 3-7 вызывает вопросы. Очевидно, что запоминать операции сравнения чисел неудобно. Почему вместо двухбуквенных сокращений нельзя использовать привычные знаки сравнения (<, > и =)? Перед тем как ответить на этот вопрос, разберёмся в истории оператора `[[`.
...
{caption: "Упражнение 3-4. Использование оператора `[` или `[[`", format: text, line-numbers: false}
```

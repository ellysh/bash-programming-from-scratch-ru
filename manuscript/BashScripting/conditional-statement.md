...
Рассмотрим пример. Предположим, что вы пишете скрипт. Он должен архивировать ваши документы. У скрипта есть три режима работы: архивация со сжатием, архивация без сжатия и разархивация. Нужное действие выбирается с помощью опции скрипта. Один из вариантов опций предлагает таблица 3-12.
...
I> При выборе формата опций и параметров скриптов всегда следуйте [POSIX-соглашению](https://www.gnu.org/software/libc/manual/html_node/Argument-Syntax.html) и [GNU-расширению](https://www.gnu.org/prep/standards/html_node/Command_002dLine-Interfaces.html) к нему.
...
Назовём наш скрипт `archiving-case.sh`. Тогда его можно выполнить одним из следующих способов:
{line-numbers: true, format: Bash}
```
./archiving-case.sh -a
./archiving-case.sh -b
...
Если передать в скрипт любые другие параметры, он завершится с ошибкой. На экран напечатается сообщение "Указана недопустимая опция".

W> При обработке ошибок в скриптах никогда не забывайте про команду exit. Код возврата после ошибки должен быть всегда ненулевым.
...
Проверка шаблонов происходит последовательно. Сначала проверяется первый, потом — второй и так далле. Если строка совпала с первым шаблоном, будет выполнен соответствующий блок. После этого остальные шаблоны и их блоки игнорируются. Bash продолжит исполнение скрипта с команды, следующей за конструкцией case.
...
Рассмотрим пример. Напишем скрипт, который архивирует PDF документы и копирует результат в специальный каталог. Для выбора действия в скрипт передаётся опция (например, `-a` для архивации и `-c` для копирования). Допустим, что копирование всегда надо выполнять после архивации. В этом случае возникнет дублирование кода.

...
{caption: "Упражнение 3-1. Использование оператора if", format: text, line-numbers: false}
```
Нам дана команда, которая проверяет файлы в каталоге с именем target на наличие в них строоки "123". Файлы, в которых она встречается, копируются в текущий каталога. Если строки в файле не обнаружено, он удаляется из каталога target.
...
Кроме сравнения строк, оператор `[[` позволяет проверять файлы на различные условия. Наиболее часто используемые из них приведены в таблице 3-13.
...
Синоним `[` ввели для лучшей читаемости кода. Благодаря ему, конструкция `if` в Bourne shell стала больше походить на свои аналоги из других языков программирования (например C). Проблема в том, что скобки и выражение в них эквивалентно вызову `test`. Этот факт легко упустить из виду, особенно имея опыт программирования на других языках. Такое несоответствие ожидаемого поведения (как конструкция языка) и реального (как вызов `test`) является источником большинства ошибок, связанных с использованием оператора `[`.
...
1. Вызовет команду `test` и передаст ей на вход переменную `$var`.
...
В случае сравнения строк задачи обеспечить обратную совместимость не стояло. Стандартный `test` вообще не поддерживает знаков `<` и `>`. Такая поддержка появилась для строк, но только в расширении POSIX-стандарта. Оттуда они были портированы в оператор `[[`.

{caption: "Упражнение 3-2. Использование оператора `[` или `[[`", format: text, line-numbers: false}
```

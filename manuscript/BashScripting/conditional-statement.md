...
Выполните этот код. На экран будет выведено сообщение, что строки равны.

Подобная проверка не слишком полезна. Обычно приходится сравнивать значение какой-то переменной со строкой. В этом случае оператор [[ выглядит так:
{line-numbers: true, format: Bash}
```
if [[ "$var" = "abc" ]]
then
...
Здесь двойные кавычки необязательны. Globbing и world splitting не выполняются при подстановке переменной в операторе [[. То есть интерпретатор никак не обрабатывает значение переменной var, а использует его как есть. Ошибка может произойти, если в строке, с которой сравнивается переменная, встречаются пробелы. Например, в следующем случае:
{line-numbers: true, format: Bash}
```
if [[ "$var" = abc def ]]
then

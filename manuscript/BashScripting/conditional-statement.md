...
Такая программа выглядит запутанной. Добавьте в неё выводы сообщений об ошибках во всех `else` случаях и читать её станет ещё сложнее. Ранний возврат решает проблему наглядности вложенных условий. Перепишем наш алгоритм с применением этой техники:
{line-numbers: false}
```
if ! ДЕЙСТВИЕ 1
then
...
В нашем скрипте резервного копирования мы можем заменить логические операторы `&&` и `||` на конструкцию `if`, но в общем случае они не эквивалентны. Рассмотрим пример. Предположим, что у нас есть следующее выражение из трёх команд A, B и C:
{line-numbers: false}
```
A && B || C
```
...
В выражении с логическими операторами поведение иное. Если A истинно, выполняется B. Далее выполнение C зависит от результата B. Если B истинно, C выполняться не будет. Если же B ложно, C исполнится. Таким образом исполнение C зависит и от резульатата A, и от результата B. В конструкции `if-else` такой зависимости нет.
...
В рассмотренных нами примерах двойные кавычки не обязательны. При подстановки переменной в выражение `[[` globbing и world splitting не происходят. То есть интерпретатор не пытается самостоятельно обработать значение переменной, а использует его как есть. Однако, если строка с которой происходит сравнение содержит пробелы, возникнет синтаксическая ошибка. Например, в следующем случае:
{line-numbers: false, format: Bash}
```
if [[ "$var" = abc def ]]
then
...
В таблице 3-11 приведены все операции сравнения строк, допустимые в конструкции `[[`.

{caption: "Таблица 3-11. Операции сравнения строк `[[`", width: "100%"}
...
|  | | |
| = или == | Поск подстроки по шаблону. Обратите внимание, что шаблон не заключается в кавычки. | [[ "$filename" = READ* ]] && echo "Имя файла начинается с READ" |
|  | | |
| =~ | Поск подстроки по [**регулярному выражению**](https://www.opennet.ru/docs/RUS/bash_scripting_guide/c11895.html). | [[ "$filename" =~ ^READ.* ]] && echo "Имя файла начинается с READ" |

Для построения сложных выражений в операторе `[[` вы можете воспользоваться логическими операциями, приведёнными в таблице 3-12.

{caption: "Таблица 3-12. Логические операции `[[`", width: "100%"}
| Операция | Описание | Пример |
| --- | --- | --- |
...
Выражения можно группировать с помощью круглых скобок. Например, следующим образом:
{line-numbers: false, format: Bash}
```
[[ (-n "$var" && "$var" < "abc") || -z "$var" ]] && echo "Строка не пустая и меньше чем abc или строка пустая"
```

Кроме сравнения строк, оператор `[[` позволяет проверять файлы на различные условия. Наиболее часто используемые операции приведены в таблице 3-13.

{caption: "Таблица 3-13. Операции проверки файлов `[[`", width: "100%"}
| Операция | Описание | Пример |
| --- | --- | --- |
...
Оператор `[[` позволяет сравнивать не только строки, но и целые числа. Таблица 3-14 демонстрирует соответствующие операции.

{caption: "Таблица 3-14. Операции сравнения целых чисел `[[`", width: "100%"}
| Операция | Описание | Пример |
| --- | --- | --- |
...
Таблица 3-13 вызывает вопросы. Очевидно, что запоминать операции сравнения чисел неудобно. Почему вместо двухбуквенных сокращений нельзя использовать привычные знаки сравнения (<, > и =)? Перед тем как ответить на этот вопрос, разберёмся в истории оператора `[[`.

Как было замечено ранее, оператор `[[` появился в Bash на замену устаревшего `test`. В первой версии Bourne shell 1979 года `test` был сторонней утилитой. Только начиная с версии System III shell 1981 года, он стал встроенной командой интерпретатора. Но встраивание `test` в shell не затронуло его синтаксис. Дело в том, что к этому времени было написано достаточно кода и новая версия интерпретатора должна была его поддерживать.

Работа с `test` в качестве внешней утилиты накладывала ограничения. Прежде они связаны с входными параметрами. Рассмотрим типичный вызов `test` для сравнения значения переменной `$var` с числом:
{line-numbers: false, format: Bash}
```
test "$var" -eq 5
```
...
Синоним `[` ввели для лучшей читаемости кода. Благодаря ему, конструкция `if` в Bourne shell стала больше походить на свои аналоги из других языков программирования (например C). Проблема в том, что выражение в скобках фактически всё равно остаётся вызовом `test`. Этот факт легко упустить из виду, особенно имея опыт программирования на других языках. Именно это является источником большинства ошибок, связанных с использованием оператора `[`.
...
Просто замените скобку на `test` и причина ошибки станет очевидна:
{line-numbers: false, format: Bash}
```
if test"$var" -eq 5
then
...
Между именем команды и её параметрами должен стоять пробел.
...
1. Вызвать команду `test` и передать ей на вход переменную `$var`.

2. Перенаправить вывод `test` в файл с именем `5` в текущем каталоге.

Подобную ошибку легко совершить и крайне сложно обнаружить. Чтобы её избежать и были введены двухбуквенные операции для сравнения чисел. Эти операции перекочевали в новый оператор `[[` Bash. По идее, ничто не мешало заменить их на знаки сравнения. Но такое решение усложнило бы портирование старого кода. Представьте, что у вас есть такой код:
{line-numbers: false, format: Bash}
```
if [ "$var" -gt 5 ]
then
...
Намного безопаснее поставить по дополнительной скобке в начале и в конце выражения, чем менять `-gt` на `>`. При такое замене высока вероятность допустить ошибку.

В случае сравнения строк задачи обеспечить обратную совместимость не стояло. Стандартный `test` не поддерживает знаков `<` и `>` ни для строк, ни длячисел. Такая поддержка появилась только в расширении POSIX-стандарта. Поэтому в оператор `[[` были добавлены эти знаки.

...
В программировании часто требуется выполнить опеределённый код в зависимости от какого-либо значения. Именно эту задачу решают условные конструкции. Мы уже познакомились с оператором `if`. У него есть альтернатива — оператор `case`.

Рассмотрим пример. Предположим, что вы пишете скрипт, который должен архивировать ваши документы. У него есть три режима работы: архивация со сжатием, архивация без сжатия и разархивация.

Для каждого режима работы вы можете ввести соответствующую опцию для скрипта. Например так, как демонстрирует таблица 3-12.
...
Значение переденной в скрипт опции можно проверять в конструкции `if`. Например так, как демонстрирует листинг 3-12.
...
В зависимости от переданной в скрипт опции (`a`, `c` или `d`), мы вызываем утилиту `bsdtar` с теми или иными опциями. Точно такое же поведение можно добиться с помощью конструкции `case`.

Листинг 3-13 демонстрирует переписанный с использованием `case` скрипт архивации.
...
В общем случае конструкция `case` позволяет сравнить переданную в неё строку со списком шаблонов. В зависимости от совпадения с шаблоном выбирается один из блоков конструкции `case`. Каждый такой блок состоит из следующих элементов:

1. Шаблон или список шаблонов, разделённых символом `|`.

2. Правая скобки `)`.

3. Набор команд, которые выполняются при совпадении шаблона и переданного в `case` слова.

4. Два знака точка с запятой `;`, которые означают окончание набора команд.

Проверка шаблонов происходит последовательно. Если переданная в конструкцию `case` строка соответствует нескольким шаблонам, будет выбран тот, который стоит в списке первым.
...
Для удобства дальнейшего рассмотрения, запишем конструкции `if` и `case` из нашего примера со скриптом архивации в общем виде. Вариант с `if` будет выглядеть следующим образом:
{line-numbers: true}
```
if УСЛОВИЕ 1
then
...
После записи конструкций в общем виде становятся очевидны их различия.

Прежде всего в `if` проверяются значения нескольких логических выражений. В общем случае эти выражения могут быть совершенно независимыми друг от друга. Наш пример с проверкой значения одной и той же переменной является частным случаем. Конструкция `case` работает с одной-единственной переданной в неё строкой.

Ещё раз обратите внимание на этот момент. Передавать в `case` логическое выражение нет никакого смысла. Это позволит вам обработать только два случая: когда выражение истинно и когда — ложно. Конструкция `if` намного удобнее для решения этой задачи.
...
Рассмотрим пример. Предположим, что наш скрипт может выполнять архивацию PDF документов пользователя и копирование сжатого файла в специальный каталог. Эти два действия можно выполнять по отдельности, задавая соответствующие опции (например, `a` для архивации и `c` для копирования). Однако перед копирование необходимо всегда выполнять архивацию.
...
Разделитель `;&` может быть полезным, но несмотря на это постарайтесь использовать его только в случае крайней необходимости. Проблема в том, что его визуально легко спутать с разделителем `;;` и неправильно прочитать код.
...
Задача решаемая конструкцией `case` близка задаче, решаемой ассоциативными массивами. Массивы устанавливают соответствие между данными (ключ-значение), а конструкция `case` между данными и командами (значение-действие). Как правило оперировать данными намного удобнее чем кодом. Их проще изменять и контролировать их корректность. Часто вы можете заменить конструкцию `case` на ассоциативный массив. По сравнению с другими языками программирования в Bash очень просто конвертировать данные в команды.

Рассмотрим пример. Предположим, что вы пишете скрипт-обёртку для утилит архивации. В зависимости от переданного в скрипт первого параметра будет вызвана либо программа `bsdtar`, либо `tar`. Листинг 3-16 демонстрирует реализацию такого скрипта с помощью конструкции `case`.
...
Скрипт можно вызвать например следующим образом:
{line-numbers: false, format: Bash}
```
...
Обратите внимание, что все параметры утилит `bsdtar` и `tar` передаются в наш скрипт-обёртку в кавычках. Благодаря этому, они попадают в позиционный параметр скрипта `$2`. Если не использовать кавычки, то они окажутся в позиционных параметрах `$2`, `$3`, `$4` и т.д. В этом случае их придётся сначала склеить в одну строку и только потом передать в утилиту архивации. Альтернативное решение — использовать специальный параметр `$@` и удалить из него опцию `$1`. Так или иначе понадобится дополнительное действие, которое мы для простоты нашего примера опустим.

Перепишем наш скрипт-обёртку с помощью ассоциативного массива. Прежде всего разберёмся в механизмах Bash для конвертации данных в команды. Суть такой конвертации заключается в том, чтобы сохранить команду и её параметры в качестве значения какой-либо переменной. Затем это значение должно быть обработано интерпретатором не как данные, а как команда. Точно так же, как если бы эту команду ввёл пользователь.

Выполним по шагам рассмотренный алгоритм конвертации. Сначала объявим переменную, значение которой соответствует команде вызова утилиты `ls`:
{line-numbers: false, format: Bash}
```
ls_command="ls"
```

Теперь попробуем передать значение этой переменной интерпретатору в качестве команды:
{line-numbers: false, format: Bash}
```
$ls_command
```

В результате выполнения вы увидите на экране содержимое текущего каталога. Что произошло? Bash выполнил подстановку параметров, в результате которой команда стала выглядеть так:
{line-numbers: false, format: Bash}
```
ls
```
...
Может возникнуть вопрос: почему в примере не используются двойные кавычки `"` при подстановке переменной `ls_command`? Чтобы ответить на этот вопрос, добавим в значение переменной параметры для вызова `ls`. Например, следующим образом:
{line-numbers: false, format: Bash}
```
ls_command="ls -l"
```
...
Проблему использования двойных кавычек при подстановке переменной, значение которой необходимо выполнить, можно решить встроенной командой интерпретатора `eval`. `eval` формирует команду для исполнения из полученных аргументов. При этом выполняется word splitting независимо от наличия двойных кавычек. В этом случае наш вызов утилиты `ls` будет выглядеть следующим образом:
{line-numbers: false, format: Bash}
```
eval "$ls_command"
```

W> Во многих источниках вы можете найти информацию о том, что обработка значений переменных в качестве команд и
использование команды `eval` — это плохая практика. В общем случае это так. Использование `eval` может привести к серьёзным ошибкам и уязвимостям. Будьте осторожны при её использовании и никогда не передавайте в `eval` данные, введённые пользователем.

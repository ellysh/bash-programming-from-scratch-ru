...
Оператор `[[` позволяет сравнивать не только строки, но и целые числа. Таблица 3-13 демонстрирует соответствующие операции.

{caption: "Таблица 3-13. Операции сравнения целых чисел `[[`", width: "100%"}
| Операция | Описание | Пример |
| --- | --- | --- |
...
Как было сказано ранее, оператор `[[` появился в Bash на замену устаревшего `test`. В первых версиях Bourne shell `test` был сторонней утилитой. Только начиная с версии System III shell 1981 года, он стал встроенной командой интерпретатора. Но встраивание `test` в shell не затронуло его синтаксис.  К этому времени было написано достаточно кода, чтобы задуматься об обратной совместимостью версий интерпретатора.

Работа с `test` в качестве внешней утилиты накладывает ограничения. Прежде всего это касается входных параметров. Рассмотрим типичный вызов `test` для сравнения значения переменной `$var` с числом:
{line-numbers: false, format: Bash}
```
test "$var" -eq 5
```

Эта команда не вызывает вопросов. В утилиту `test` передаются три разделённые пробелами параметра: значение переменной `$var`, ключ `-eq` и число 5.

Вызов `test` можно комбинировать с конструкцией `if`:
{line-numbers: false, format: Bash}
```
if test "$var" -eq 5
then
...
Для большей наглядности в Bourne shell был введён синоним `[` для `test`. Единственное отличие между ними - это требование закрывающей скобки `]`, которая не нужна для `test`. С помощью синонима мы можем переписать наше выражение `if`:
{line-numbers: false, format: Bash}
```
if [ "$var" -eq 5 ]
then
...
С это момента и возникают вопросы. Например, почему между скобкой `[` и следующим далее символом `"` нужен пробел? Если помнить, что `[` - это `test`, ответ очевиден. Потому что между именем утилиты и её первым параметром должен быть пробел.

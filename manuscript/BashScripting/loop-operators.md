...
Команда двоеточие является полным аналогом `true`. Она нужна для [совместимости](https://stackoverflow.com/questions/3224878/what-is-the-purpose-of-the-colon-gnu-bash-builtin) с Bourne shell, в котором `true` и `false` не было. Все три команды (`:`, `true`, `false`) включены в POSIX-стандарт.

Рассмотрим пример использования бесконечного цикла. Предположим, что вам нужно периодически выводить статистику об использовании дискового пространства на вашей системе.
...
// TODO: Упомянуть про команду `watch` (проверить есть ли она в MSYS2).

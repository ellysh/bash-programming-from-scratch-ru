...
| --- | --- | --- | --- |
| 1 | Последнее число в генерируемой последовательности, котоаря начинается с единицы. | `seq 5` | 1 2 3 4 5 |
...
В выводе утилиты `seq` по умолчанию в качестве разделителя используется управляющий символ перевод строки `\n`. 
Его можно заменить на любой другой с помощью опции `-s`. Перевод строки входит в обычный список разделетиелей специальной переменной `IFS`. Поэтому  в конструкции можно использовать `seq` без опции `-s`.

I> В таблице 3-20 перевод строки был замещен на пробел в столбце "Результат" для удобства.
...
Преимущество этого скрипта в том, что в нём используются только встроенные операторы Bash. Для их выполнения не требуется создавать новые процессы. Поэтому в общем случае такое решение будет эффективнее.

// TODO: Рассмотреть скрипт по шагам.

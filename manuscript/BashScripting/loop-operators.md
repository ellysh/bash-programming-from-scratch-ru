...
{caption: "Листинг 3-21. Текстовый файл со списком контактов", line-numbers: true, format: text}
![`contacts.txt`](code/BashScripting/contacts.txt)

Теперь напишем скрипт для чтения текстового файла и записи его содержимого в массив. Очевидно, что для этой задачи нам понадобится цикл. Перед начало чтения файла, мы не знаем сколько в нём строк. Поэтому выберем цикл `while`.

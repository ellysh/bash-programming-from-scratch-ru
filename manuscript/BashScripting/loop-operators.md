...
Выполнение while начинается с проверки УСЛОВИЯ. Если команда УСЛОВИЯ вернула нулевой код, считается что оно истинно. В этом случае выполняется ДЕЙСТВИЕ. Дальше опять проверяется УСЛОВИЕ. Если оно по-прежнему истинно, снова выполняется ДЕЙСТВИЕ. Цикл прервётся тогда, когда УСЛОВИЕ станет ложным.

Используйте цикл while, когда количество итераций неизвестно заранее. Например, при активном ожидании какого-то события.
...
Утилита ping ожидает ответ от сервера, пока её не прервёт пользователь. С помощью опции `-W` зададим таймаут на ожидание. Одной секунды будет достаточно. Получится такая команда:
{line-numbers: false, format: Bash}
```
ping -c 1 -W 1 google.com
```

У нас готово условие для конструкции while. Запишем её целиком:
{line-numbers: true, format: Bash}
```
...
Команда true всегда возвращает истину. То есть её код возврата ноль. У true есть симметричная команда false, которая всегда возвращает единицу, то есть ложь.
...
Занятое (Used) и свободное (Available) дисковое пространство указано в байтах. Добавим в вызов утилиты опцию `-h`. Тогда вместо байтов получим килобайты, мегабайты, гигабайты и терабайты. Также добавим опцию `-T`. С ней выводится тип файловой системы для каждого диска. Вывод утилиты станет таким:
{line-numbers: true, format: Bash}
```
$ df -hT
...
Напишем бесконечный цикл, в теле которого вызывается утилита df. Так получится простейший скрипт для наблюдения за файловой системой. Скрипт приведён в листинге 3-20.
...
Опция `-n` утилиты watch задаёт интервал между вызовами команды. Команда для исполнения указывается вторым параметром.

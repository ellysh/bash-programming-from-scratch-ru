...
Добавим к нашему вызову утилиты `ping` опцию `-W`, чтобы задать таймаут ожидания ответа от сервера. Одной секунды должно быть достаточно. Получится следующий вызов:
{line-numbers: false, format: Bash}
```

...
Встроенная команда continue прекращает исполнение текущей итерации цикла. Цикл не завершится. Он продолжит выполняться со следующей итерации.

Рассмотрим пример. Предположим, что надо рассчитать сумму положительных чисел в массиве. Отрицательные числа нас не интересуют. С помощью конструкции if проверим знак в теле цикла. Получим скрипт, как в листинге 3-35.
...
Если `element` больше нуля, его значение добавляется к результирующей сумме `sum`.

Воспользуемся командой continue, чтобы получить то же самое поведение. Результат приведён в листинге 3-36.
...
Мы инвертировали условие конструкции if. Теперь оно истинно для отрицательных чисел. В этом случае вызывается команда continue. Она прерывает текущую итерацию цикла. Операция сложения после if не выполнится. Вместо этого начнётся следующая итерация со следующим элементом массива.

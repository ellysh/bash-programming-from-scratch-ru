...
Оператор цикла `for` используется в случаях, когда количество итераций заранее известно. У него есть две формы. Первая позволяет последовательно обработать каждое слово в некоторой строке. Вторая форма в качестве условия выхода из цикла использует арфиметическое выражение.
...
Перед выполнением `for` Bash делает все необходимые подстановки. Это значит, что если вместо СТРОКИ вы указали команду, она будет выполнена и на её место будет подставлен её вывод. Если вы указали шаблон — он будет развёрнут. Дальше СТРОКА разделяется на слова. В качестве разделителей используются символы из зарезервированной переменной `IFS`. Затем происходит выполнение первой итерации цикла. При этом первое слово, прочитанное из СТРОКИ, будет доступно в теле цикла как значение ПЕРЕМЕННОЙ.

Начнём с самого простого примера. Напишем скрипт, который обрабатывает каждое слово в строке, переданной ему в первом позиционном параметре. Его код приведён в листинге 3-23.
...
Обратите внимание, что заключать позиционный параметр `$1` в кавычки не следует. Иначе цикл выполнит только одну итерацию, в которой переменная `word` будет равна всей строке.
...
Немного усложним задачу. Предположим, что пользователь передаёт в скрипт список путей (например, к файлам), которые разделённы запятыми. В путях могут встречаться пробелы. Чтобы решить эту задачу, нам понадобится переопределить разделитель в специальной переменной `IFS`. Результат приведён в листинге 3-24.
...
В этом скрипте мы присвоили переменной `IFS` единственный символ запятая `,`. Поэтому скрипт `for` при обработке строки из параметра `$1` будет ориентироваться именно на запятые и игнорировать пробелы. Таким образом на каждой итерации в переменную `path` будет записываться один путь.
...
Цикл `for` часто используется для прохода по ряду чисел. Этот ряд числе можно получить с помощью подстановки фигурных скобок. Например так:
{line-numbers: true, format: Bash}
```
for i in {1..9}
do
...
Подобный цикл можно использовать для прохода по элементам индексируемого массива, если вы уверены что в нём нет пропусков. Например так:
{line-numbers: true, format: Bash}
```
...
Более надёжный прохода по массиву с возможными пропусками выглядит так:
{line-numbers: true, format: Bash}
```
array=(Alice Bob Eve Mallory)
...
// TODO: Переменные нельзя использовать в подстановке {}, только через workaround.

### Обработка списка файлов

Рассмотрим пример. Предположим, что вам нужно написать скрипт, который бы выводил тип всех файлов в текущем каталоге. Для вывода типа файла воспользуемся уже знакомой нам утилитой `file`. Единственное, что нам нужно сделать, это правильно написать условие цикла `for`.

Одна из самых частых ошибок начинающих программистов при написании циклов `for` это пренебрежение шаблонами. Многие в качестве СТРОКИ используют подстановку вывода утилит `ls` и `find`. Например так:
{line-numbers: true, format: Bash}
```
for filename in $(ls)
...
1. Word splitting приведёт к тому, что содержащие пробелы имена файлов и каталогов, будут обработан по частям за несколько итераций. 

2. Если имя файла содержит символ звёздочка `*`, перед очередной итерацией цикла будет выполнена подстановка шаблонов. Её результат будет записан в переменную `filename`. Таким образом её правильное значение будет утеряно.
...
Правильно всегда использовать шаблоны (globbing) в цикле `for`, если вам нужно перебрать имена файлов. В нашем случае условие цикла будет выглядеть так:
{line-numbers: false, format: Bash}
```
for filename in *
```

Полная версия скрипта приведена в листинге 3-23

{caption: "Листинг 3-23. Скрипт для вывода типов файлов", line-numbers: true, format: Bash}
![`for-file.sh`](code/BashScripting/for-file.sh)

Не забывайте про двойные кавычки при подстановке переменной `filename`.
...
Скрипт из листинга 3-23 мы могли бы заменить следующей командой `find`:
{line-numbers: false, format: Bash}
```
find . -maxdepth 1 -exec file {} \;
```

Очевидно, что это решение нашей задачи более предпочтительно, чем использование цикла `for`. Оно компактнее и быстрее, посольку содержит меньше операций.

Когда же использование цикла `for` оправдано для прохода по списку имён файлов или каталогов? Утилита `find` хорошо справляется, когда для обработки найденных ею объектов достаточно одной команды без каких-либо дополнительных проверок. В противном случае вызов `find` становится громоздким и неудобным для чтения. Тогда его стоит заменить на цикл `for`.

В скрипте из листинга 3-23 также можно использовать оператор `while` вместо `for`. Для этого ей на вход можно передать вывод утилиты `find`, при вызове которой обязательно надо использовать опцию `-print0`. Листинг 3-24 демонстрирует вариант скрипта с `while`.

{caption: "Листинг 3-24. Скрипт для вывода типов файлов", line-numbers: true, format: Bash}
![`while-file.sh`](code/BashScripting/while-file.sh)

Рассмотрим этот скрипт подробнее. Прежде всего может возникнуть вопрос, почему мы переобъявили зарезервированную переменную `IFS`, присвоив ей пустое значение? Если этого не сделать, то вывод команды `find` будет разделён пробелами, знаками табуляции и перевода строки. Таким образом имена файлов и каталогов, содержаищие эти символы, будут обработаны неправильно.

Второй вопрос может вызвать опция `-d` команды `read`. С её помощью можно переопределить символ, по которому `read` будет разделять текст на входе. Таким образом в переменную `filename` будет записана вся строка до очередного разделителя.

В нашем примере разделитель пустой. Это значит, что он равен NUL-символу. Вы также можете указать его явно. Например так:
{line-numbers: false, format: Bash}
```
while IFS= read -r -d $'\0' filename
```

Благодаря опции `-d ''`, команда `read` теперь сможет обработать вывод утилиты `find`, в которой в качестве разделителя используется NUL-символ (опция `-print0`).

Обратите внимание, что вы не можете определить NUL-символ в качестве разделителя через зарезервированную переменную `IFS`. Другими словами следующий вариант не заработает:
{line-numbers: false, format: Bash}
```
...
Последний вопрос по скрипту из листинга 3-24 связан с подстановкой процесса. Почему мы не использовали подстановку команды? Например так:
{line-numbers: true, format: Bash}
```
...
Проблема в том, что вы не можете перенаправлять результат выполнения команды таким образом. Оператор `<` связывает стандартный поток ввода с указанным файловым дескриптором. Но в случае подстановки команды никакого дескриптора нет. Bash просто вызовет утилиту `find` и подставит её вывод. Если же использовать подстановку процессов, вывод `find` будет сохранён во временном файле, у которого будет дескриптор. Поэтому перенаправление отработает корректно.

У подстановки процессов есть одна проблема. Она специфична для Bash и не включена в POSIX-стандарт. Если вам важно следование стандарту, используйте конвейер. Листинг 3-24 демонстрирует это решение.

{caption: "Листинг 3-25. Скрипт для вывода типов файлов", line-numbers: true, format: Bash}
![`while-file-pipe.sh`](code/BashScripting/while-file-pipe.sh)

Если вам требуется пройти по файлам, полученным в результате сложного поиска с использованием утилиты `find`, всегда предпочитайте цикл `while` вместо `for`, как в примере из листинга 3-24. С помощью `while` вы сможете разделить результаты поиска NUL символом. Это позволит избежать ошибок с именами файлов и каталогов, использующих пробелы и другие специальные символы.

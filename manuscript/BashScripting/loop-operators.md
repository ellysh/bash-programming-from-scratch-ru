...
В Bash есть два оператора цикла: while и for. Начнём с оператора while. Он проще чем for.

Синтаксис while напоминает условный оператор if. В общем виде его можно записать так:
...
Запись оператора в одну строку выглядит так:
{line-numbers: false, format: Bash}
```
...
В качестве входного параметра утилита ping принимает [URL](https://ru.wikipedia.org/wiki/URL) или [IP-адрес](https://ru.wikipedia.org/wiki/IP-адрес) целевого **хоста**. Хостом называется любой подключённый к сети компьютер.
...
Запись в одну строку похожа на while:
{line-numbers: false}
```
...
Выбирайте форму while или until в зависимости от условия цикла. Старайтесь составлять условия без отрицаний. Отрицания усложняют чтение кода и приводят к ошибкам.

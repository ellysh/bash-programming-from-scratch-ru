...
Немного усложним задачу. Предположим, что скрипт получает на вход список путей. Они разделены запятыми. В путях могут встречаться пробелы. Чтобы правильно обработать ввод, придётся переопределить переменную `IFS`.

Листинге 3-24 демонстрирует цикл for для обработки списка путей.
...
В этом случае кавычки для строки с путями обязательны. Если их опустить и заменить в скрипте параметр `$1` на `$@`, возникнет ошибка. Word splitting произойдёт до переобъявления переменной `IFS` в скрипте. Поэтому строка с путями разделится пробелами.

Если в одном из путей встречается запятая, опять же возникнет ошибка.

Цикл for позволяет пройти по элементам индексируемого массива. Это работает так же, как перебор слов в строке. Листинге 3-25 демонстрирует пример.
...
Полная версия скрипта приведена в листинге 3-27
...
Скрипт из листинга 3-27 мы могли бы заменить следующей командой `find`:
{line-numbers: false, format: Bash}
```
...
Скрипт из листинга 3-27 можно переписать с использованием оператора `while`. Для этого ему на вход следует передать вывод утилиты `find`. При вызове `find` обязательно надо использовать опцию `-print0`. Листинг 3-28 демонстрирует такой вариант скрипта.
...
Последний вопрос по скрипту из листинга 3-28 связан с подстановкой процесса. Почему мы не использовали подстановку команды? Например так:
{line-numbers: true, format: Bash}
```
...
У подстановки процессов есть одна проблема. Она специфична для Bash и не включена в POSIX-стандарт. Если вам важно следовать стандарту, используйте конвейер. Листинг 3-29 демонстрирует это решение.
...
Предположим, вы пишете скрипт для расчёта факториала. Если число, для которого он рассчитывается заранее известно, вы можете использовать первую форму оператора `for`. Например так, как демонстрирует листинг 3-30.
...
С помощью утилиты `seq` мы сможем написать параметризуемый скрипт расчёта факториала. Он приведён в листинге 3-31.
...
Перепишем скрипт из листинга 3-31, заменив вызов утилиты `seq` в цикле `for` на арифметическое выражение. Результат приведён в листинге 3-32.
...
Для примера предположим, что вы пишете скрипт поиска элемента индексируемого массива с определённым значением. В этом случае как только вы его нашли, нет смысла продолжать цикл. Можно сразу выйти из него. Листинг 3-33 демонстрирует такой скрипт.
...
Команда `break` используется, когда необходимо продолжить выполнение скрипта после оператора цикла. Как, например, в листинге 3-33 результаты поиска выводятся в следующей за циклом конструкции `if`.

В некоторых случаях вместо команды `break` предпочтительнее использовать `exit`. Это оправдано, когда выполнение скрипта после цикла не имеет смысла. Например, в случае обнаружения ошибки или вывода результата сразу в цикле. Перепишем скрипт из листинга 3-33 с использованием команды `exit` вместо `break`. Результат приведён в листинге 3-34.
...
Результат работы скриптов из листинга 3-23 и 3-24 полностью совпадает. Использование команды `exit` позволило написать более компактный код без вспомогательной переменной `is_found` и конструкции `if`. Поэтому всегда рассматривайте такой вариант выхода из цикла.
...
Предположим, что вы обрабатываете массив чисел. В нём вас интересуют только положительные числа, сумму которых надо рассчитать. В этом случае можно воспользоваться конструкцией `if` для проверки знака. Возможный вариант решения приведён в листинге 3-35.
...
Этого же поведения можно добиться с помощью команды `continue`, как демонстрирует листинг 3-36.

...
Цикл `while` часто используется для обработки стандартного потока ввода. Рассмотрим эту возможность на примере. Напишем скрипт, который будет читать ассоциатинвый массив из текстового файла.

Листинг 3-10 демонстрирует скрипт для хранения и вывода контактов. В нём инициализируются все элементы ассоциативного массива. Это неудобно для редактирования и сопровождения. Было бы лучше хранить список контактов в обычном текстовом файле. Его возможный формат приведён в листинге 3-21.
...
Теперь напишем скрипт для чтения текстового файла и записи его содержимого в массив. Очевидно, что для этой задачи нам понадобится цикл. Перед начало чтения файла, мы не знаем сколько в нём строк. Поэтому воспользуемся циклом `while`.

Для чтения строк файла мы можем применить встроенную команду интерпретатора `read`. Она читает строку из стандартного потока ввода и сохраняет её в переменную. Например, следующим образом:
{line-numbers: false, format: Bash}
```
read var
```

В этом случае после запуска команды на исполнение вы должны ввести строку, которая будет сохранена в переменной с именем `var`. Если имя не указано, строка будет сохранена в зарезервированной переменной `REPLY`.

Команда `read` позволяет экранировать некоторые введённые символы с помощью обратного слэша `\`. Чтобы отключить эту функцию, используйте параметр `-r`.

Если команде `read` передать несколько имён переменных, следующих через пробел, то вводимый пользователем текст будет поделён на строки. В качестве разделителей между ними будут использоваться символы зарезервированной переменной `IFS`. По умолчанию это пробел, знак табуляции и перевода строки.
...
В этом примере мы использовали в качестве разделителя запятую `,`. Поскольку она не встречается ни в пути, ни в имени файла, ввод пользователя был обработан корректно. Строка `~/My Documents` попала в переменную `path`, а `report.txt` в `file`.

Команда `read` читает данные со стандартного потока ввода. Это значит, что вы можете перенаправить её на вход содержимое кого-либо файла. Например, прочитать первую строчку файла `contacts.txt` из листинга 3-21 можно следующей командой:
{line-numbers: false, format: Bash}
```
...
Однако, это не сработает. Произойдёт зацикливание, потому что команда `read` всегда будет читать только первую строчку файла и возвращать нулевой код возврата.

Чтобы цикл `while` последовательно прошёл по всем строкам из стандартного ввода, его нужно записать в следующеё форме:
{line-numbers: true, format: Bash}
```
while УСЛОВИЕ
do
...
Последний шаг, который нам остался, заключается в инициализации ассоциативного массива из прочитанных переменных `name` и `contact`. Конечный вариант скрипта для чтения и работы с контактами приведён в листинге 3.22

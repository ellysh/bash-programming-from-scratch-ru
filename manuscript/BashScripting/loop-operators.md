...
Начнём наше знакомство с циклам с оператора `while`. Его синтаксис напоминает условную конструкцию `if`. В общем виде его можно записать следующим образом:
{line-numbers: true}
```
while УСЛОВИЕ
do
...
Напишем скрипт, который ожидает ответ от сервера. Как только ответ получен, скрипт выводит сообщение на экран и завершает свою работу. Для обращения к серверу воспользуемся утилитой [`ping`](https://ru.wikipedia.org/wiki/Ping). Она посылает [ICMP](https://ru.wikipedia.org/wiki/ICMP) echo-запросы на удалённый сетевой хост и получает от него echo-ответы. Вы можете вызвать утилиту `ping` из командной строки следующим образом:
{line-numbers: false, format: Bash}
```
ping google.com
```

После её запуска вы увидите примерно следующий вывод:
...
Наш скрипт должен запускать утилиту `ping` таким образом, чтобы она посылала только один echo-запрос. Для этого воспользуемся её параметром `-c`. Он позволяет указать число отправляемых пакетов. Тогда вызов утилиты будет выглядеть так:
{line-numbers: false, format: Bash}
```
ping -c 1 google.com
```
...
Теперь запишем конструкцию `while`, в которой вызов утилиты `ping` будет выполнять роль условия. Получим следующий результат:
{line-numbers: true, format: Bash}
```
...
В условии цикла мы инвертировали результат вызова `ping`. Это значит, что пока утилита возвращает отличный от нуля код, будет выполняться тело цикла. В качестве действия вызывается утилита `sleep`. Она останавливает выполнение скрипта на указанное кодичество секунд. В нашем примере остановка длится одну секунду.

// TODO: Дать конечный вариант скрипта с `echo` в конце.

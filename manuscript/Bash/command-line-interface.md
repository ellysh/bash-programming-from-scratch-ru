...
При сочетании GNU утилит возникает проблема. Как передавать выход одной программы на вход другой? У этой задачи есть несколько решений. Если объём данных небольшой, можно использовать буфер обмена. Просто выделите мышью вывод команды, затем наберите следующую команду и вставьте содержимое из буфера обмена. Этот метод перестаёт работать, когда копируемые данные занимают несколько экранов. В этом случае легко что-то упустить и скопировать не всё. Так же этот метод не удобен, когда цепочка команд становится больше двух. Было бы хорошо автоматизировать эту передачу данных.

Вместо буфера обмена для временного хранения данных можно воспользоваться файловой системой. Предположим, что нам нужно сохранить список файлов, найденных командой `find`. Для этого воспользуемся символом `>`:
{line-numbers: false}
```
find / -path */doc/* -name README > readme_list.txt
```
В результате выполнения этой команды на жёстком диске будет создан файл `readme_list.txt`. В него будет записан весь вывод команды `find` в том же самом виде, в каком он бы вывелся на экран. Если файл с именем `readme_list.txt` уже существует в текущем каталоге, его содержимаое будет удалено и перезаписано выводом `find`.

...
Мы рассмотрели, как с помощью параметра `-exec` команды `find` указатать действие над каждым найденным файлом. Возможно ли сделать то же самое, но с помощью конвейеров?
...
Если мы говорим об интерпретаторе Bash, ситуация меняется. Предположим, что мы запускаем утилиту `ls`. Bash не имеет доступа к промежуточным результатам её работы. Он получает её вывод целиком в стандартном потоке вывода. Если вы хотите скопировать файлы, имена которых выдала `ls`, просто конвейер не подойдёт:
{line-numbers: false}
```
ls /usr/share/doc/bash | cp ~
```

Такой вызов завершится ошибкой от утилиты `cp`, потмоу что она не читает стандартный поток ввода, а ожидает имя копируемого файла в первом параметре. Наша задача свелась к тому, как сконструировать строку параметров из текста стандартного потока вывода. Утилита `xargs` делает именно то, что нам нужно:
{line-numbers: false}
```
ls /usr/share/doc/bash | xargs cp -t ~
```

В этом вызове стандартный поток вывода команды `ls` передаётся на вход `xargs`. Далее `xargs` вызывает команду `cp` с параметром `-t ~`. Он означает копирование в указанный каталог. После параметра `xargs` подставит каждый файл из вывода `ls`. В результате вызов `cp` будет выглядеть как:
{line-numbers: false}
```
cp -t ~ README LICENSE
```

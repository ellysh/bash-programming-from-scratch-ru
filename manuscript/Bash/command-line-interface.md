...
Все GNU утилиты строятся вокруг текстовых данных. Они, как правило, получают на вход текстовые файлы или строки. Результат работы чаще всего выводится на экран также в текстовом виде. Имея универсальный формат данных, очень удобно комбинировать утилиты друг с другом. Таким образом из ограниченного набора узкоспециализированных инструментов можно составить решение для достаточно сложных задач.
...
Для временного хранения данных можно воспользоваться файловой системой. Это намного удобнее буфера обмена. Во-первых снимается ограничение на количество строк. Во-вторых не требуется ручных операций копирования в буфер. Рассмотрим пример. Предположим, что нам для отчёта нужно сохранить список файлов, найденных командой `find`. Для этого воспользуемся командой `1>`:
{line-numbers: false}
```
...
Что означает команда `1>`? Это перенаправление стандартного потока вывода. В Unix окружении есть всего три стандартных потока. Они приведены в таблице 2-10.
...
I> Команду перенаправления стандартного потока вывода можно сократить до одного знака `>`.
...
I> Перенаправления стандартного потока ввода можно сократить до одного знака `<`.

Иногда возникает необходимость сохранять текущее содержимое файла и дописывать вывод команды в его конец. Представьте, что у нас есть жёсткий диск с большим числом лог файлов. Они хранятся в катологах, соответствующих году и месяцу их создания. Нам требуется найти лог файлы с определённым именем только за 2018 и 2019 годы. Поиск на всём жёстком диске займет много времени. Поэтому логичнее будет запустить две команды `find` только в интересующих нас каталогах. 

Предположим, что нас интересуют логи с расширением `log`. Они хранятся  в каталоге `/log`. Тогда команды поиска будут выглядеть следующим образом:
{line-numbers: false}
```
find /logs/2019 -name "*.log" > result.txt
find /logs/2018 -name "*.log" > result.txt
```
Мы знаем, что перенаправление вывода символом `>` приведёт к перезаписи файла второй командой. Чтобы этого не произошло, воспользуемся символом `>>`:
{line-numbers: false}
```
find /logs/2019 -name "*.log" > result.txt
find /logs/2018 -name "*.log" >> result.txt
```
В результате первая команда `find` создаст новый файл или очистит существующий и запишет в него свой результат. Вторая команда допишет свои данные в конец этого файла.

I> Если указанного файла не существует, оба варианта перенаправления стандартного потока вывода создадут его.



Предположим, что нам требуется выполнить несколько команд `find`. Затем их общий вывод надо передать на вход финальной команды `grep`. Вы могли бы вывести результат каждого `find` в отдельный файл, а затем склеить эти файлы в один с помощью `cat`:
{line-numbers: false}
```
find / -path */doc/* -name README > readme_list.txt
find / -path */doc/* -name html > html_list.txt
...
cat readme_list.txt html_list.txt .. > list.txt
```

// TODO: Рассказать про ввод `grep < `


// TODO: Рассказать про стандартные потоки

...
По умолчанию утилита `xargs` добавляет параметры, переложенные из стандартного потока ввода, в конец указанной команды. Могут быть ситуации, когда требуется указать место их подстановки. В этом случае можно воспользоваться параметром `-I`:
{line-numbers: false}
```
ls /usr/share/doc/bash | grep html | xargs -I % cp % ~
```

Параметр `-I %` сообщает утилите `xargs`, что подстановку надо совершить в место символа `%` в следующем далее вызове. В нашем случае сформируется следующая команда `cp`:
{line-numbers: false}
```
cp bash.html bashref.html ~
```

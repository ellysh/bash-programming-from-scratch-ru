...
Команда `info` выводит справку по Bash. Следующий далее символ `|` означает, что вывод команды слева передаётся на вход программы справа. Таким образом `grep` получит текст справки и выведет все вхождения строки "GNU". Параметр `-n` добавит в вывод `grep` номера строк.

Другой частый случай использования конвейера - команда `ls`. Если вам надо вывести имена файлов в указанном каталоге, в которых встречается некоторое слово, вы можете воспользоваться командой `find`. Но вместо этого можно сочетать команды `ls` и `grep`. Результирующая команда получится короче:
{line-numbers: false}
```

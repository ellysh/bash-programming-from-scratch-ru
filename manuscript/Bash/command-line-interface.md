...
Таблица 2-2 представляет собой вывод команды `mount` с иллюстрации 2-10, разделённый на столбцы.
...
Таблица 2-2 может вызвать недоумение у Windows-пользователей. Корневым каталогом для Bash является `C:/MinGW/git`. Но поскольку все точки монтирования находятся в нём, то диск `C` также является его подкаталогом. С точки зрения файловой системы Windows зависимость обратная: `C:/MinGW/git` является подкаталогом диска `C`. Такое противоречие не вызывает проблем, поскольку Bash и все его утилиты работают в Linux структуре каталогов. Если же вы запускаете обычное Windows приложение, оно будет работать в структуре каталогов Windows. Обе структуры одновременно существуют в вашей системе. Каждое запускаемое приложение выбирает, какой из них ему пользоваться. До тех пор пока приложение пользуется только одной структурой, а не обоими сразу, всё работает корректно.

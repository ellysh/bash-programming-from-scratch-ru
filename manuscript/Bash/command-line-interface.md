...
Последняя команда для навигации по файловой системе, которую мы рассмотрим, позволяет найти файлы и каталоги. Она называется `find`. Если вы запустите её без параметров, она выведет всё содержимое текущего каталога. Иллюстрация 2-13 демонстрирует результат выполнения `find` для домашнего каталога пользователя `~`.

{caption: "Иллюстрация 2-13. Результат выполнения команды `find`", height: "30%"}
![Результат cat](images/Bash/find-command.png)

Первым параметром команды `find` можно указать относительный или абсолютный каталог, в котором она должна быть исполнена. Например, вызов `find /` исполнит команду в корневом каталоге. Вторым параметром указывается выражение из одного и более элементов. Оно определяет, какие именно файлы и каталоги попадут в вывод команды. Наиболее часто применяемые выражения приведены в таблице 2-3.

{caption: "Таблица 2-3. Часто применяемые выражения команды `find`", width: "100%"}
...
| `-mtime число` | Искать только файлы и каталоги, которые были изменены в течении указанного количества дней назад. | `find -type d` |

...
В окне терминала сразу после запуска выводятся две строчки (см. иллюстрация 2-3):
```
ilya.shpigor@DESKTOP-1NBVQM$ MinGW64 /
$
```

Рассмотрим их подробнее. Первая строка начинается с имени пользователя. В моём случае это `ilya.shpigor`. Далее через символ `@` следует имя компьютера. Его можно изменить в настройках ОС. Затем идёт `MinGW64`. Это название платформы, на которой запущен Bash. После этого стоит символ `/`. Он означает текущий абсолютный путь. Остановимся на этом пункте поподробнее.

В Проводнике Windows есть адресная строка. В ней отображается полный путь до открытой в данный момент папке. Например, на иллюстрации 2-4 текущим путём является `This PC > Local Disk (C:) > MinGW`. Это означает, что открыта папка `MinGW`, которая хранится на локальном диске `C:`. Локальный - означает физически подключённый к вашему компьютеру. Ту же самую информацию выводит и Bash. Но между путём в Bash и Проводнике есть важно различие.

{caption: "Иллюстрация 2-4. Окно Проводника Windows"}
![Окно Bash](images/Bash/explorer.png)

...
Рассмотрим каждую из команд таблицы 2-1. Мы только что запустили терминал. Первым делом всегда полезно уточнить текущий каталог. Терминал MinGW выводит эту информацию перед приглашением командной строки. Преположим, что этого не происходит. Воспользуемся командой `pwd`. Мы получим результат, приведённый в верхней части иллюстрации 2-5. Он означает, что сейчас мы находимся в корневом каталоге `/`. Если вы укажете ключ `-W`, то будет выведен текущий путь в структуре каталогов Windows.
...
Обратите внимание, что в полученом нами выводе нет подкаталогов `/c` и `/z`. На самом деле эти каталоги представляют собой точки монтирования соответствующих дисков. То есть в структуре каталогов Windows этих путей нет. Они создаются в окружении Linux, чтобы вы смогли перейти в корневые каталоги дисков `C` и `Z`. Утилита `ls` читает содержимое каталога в окружении Windows, поэтому и не отображает точки монтирования.

Когда к вашему компьютеру подключено несколько дисков, может быть полезно вывести на экран их точки монтирования. Эту задачу выполняет команда [`mount`](https://ru.wikipedia.org/wiki/Mount), запущенная без параметров. Иллюстрация 2-8 демонстрирует результат выполнения этой команды в моём случае.

{caption: "Иллюстрация 2-8. Результат выполнения команды `mount`"}
...
| --- | --- | --- |
| `C:/MinGW/git` | `/` | `ntfs` | `binary,noacl,auto` |
| `C:/MinGW/git/usr/bin` | `/bin` | `ntfs` | `binary,noacl,auto` |
| `C:/Users/ILYA~1.SHP/AppData/Local/Temp` | `/tmp` | `ntfs` | `binary,noacl,posix=0,usertemp` |
| `C:` | `/c` | `ntfs` | `binary,noacl,posix=0,user,noumount,auto` |
| `Z:` | `/z` | `ntfs` | `binary,noacl,posix=0,user,noumount,auto` |

Таблица 2-2 может вызвать недоумение у Windows-пользователей. Корневым каталогом для Bash является `C:/MinGW/git`. Но поскольку все точки монтирования находятся в нём, то диск `C` также является его подкаталогом. С точки зрения файловой системы Windows зависимость обратная: `C:/MinGW/git` является подкаталогом диска `C`. Такое противоречие не вызывает проблем. Как мы уже выяснили, путь `/c` является точкой монтирования. То есть такой каталог в окружении Windows не сушествует. Его можно рассматривать просто как [**ссылку**](https://ru.wikipedia.org/wiki/Символическая_ссылка) на диск `C`.
...
Команда`cd` работает как с абсолютными путями, так и относительными. Относительные пути как правило короче и удобнее в наборе. Только что мы перешли в каталог `/mingw64` по его относительному пути `mingw64`. Вопрос - как нам вернуться обратно в корневой каталог? Можно воспользоваться его абсолютным путём: `cd /`. Либо специальным относительным путём `..`, который означает каталог на уровень выше по иерархии. Тогда команда перехода будет выглядеть как `cd ..`.

I> Кроме `..` в Bash есть еще два специальных пути. Путь `.` означает текущий каталог. То есть если вывполните `cd .`, то ничего не произойдёт. Вы останетесь в том же текущем каталоге. Путь `~` означает домашний каталог пользователя. Выполнив `cd ~`, вы окажетесь например в `/c/Users/ilya.shpigor`. Причём в приглашении командной строки будет указан текущий каталог `~`.

Если вы воспользуетесь командой `ls`, то обнаружите каталог `doc`, а в нём `git-credential-manager`. Перейдите в него командой `cd doc/git-credential-manager`. По `ls` вы обнаружите там два файла: `LICENSE.txt` и `README.md`. Нас интересует второй. Выведите его содержимое на экран командой ` cat README.md`. Её результат приведён на иллюстрации 2-10.

{caption: "Иллюстрация 2-10. Результат выполнения команды `cat`"}

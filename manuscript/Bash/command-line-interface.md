...
rm ~/docs/reports/2019/report*.txt
```
Если вы попробуете удалить файл с доступом только на чтение, будет выведено предупреждение как на иллюстрации 2-22.

{caption: "Иллюстрация 2-22. Предупреждение при удалении файла с доступом на чтение", width: "100%"}
...
На иллюстрации 2-22 обратите внимание на команду `ls`, запущенную с параметром `-l`. Она выводит таблицу с подробной информацией о файлах и каталогах. Каждому из них соответствует одна строка таблицы. В первом её столбце указаны права доступа. У файла `report.txt` права доступа равны `-r--r--r--`. Рассмотрим что означает эта запись.
...
В Unix вся информация о доступе записывается одной строкой. Когда к файлу имеют полный доступ все пользователи, эта строка выглядит как `-rwxrwxrwx`. Для каталога с такими же правами первый символ тире заменяется на d: `drwxrwxrwx`.

В таблице 2-6 приведена расшифровка каждого символа в строке прав доступа.

{caption: "Таблица 2-6. Значение символов в строке прав доступа", width: "50%"}
...
Позиция символа в строке прав доступа определяет пользователей, к которым соответствующее право относится. Возможные варианты приведены в таблице 2-7.

{caption: "Таблица 2-7. Значение позиции букв в строке прав доступа", width: "70%"}

| d | rwx | rwx | rwx |
| --- | --- | --- | --- |
| | Владелец файла или каталога. По умолчанию это тот, кто его создал. | Группа пользователей привязанная к файлу. По умолчанию, это группа к которой относится владелец. | Все остальные пользователи кроме владельца и группы, привязанной к файлу. |
...
Вернёмся к иллюстрации 2-22. Права доступа `-r--r--r--` к файлу `report.txt` означают, что его могут читать все пользователи. Изменять и исполнять не может никто. Права `-rw-r--r--` файла `report.txt` означают, что его могут читать все, а изменять только владелец.

#### Запуск файлов

// TODO: Рассмотреть права файлов и каталогов:

...
Не забывайте про имена файлов, которые могут содержать символы перевода строки. Чтобы обработать их корректно, добавим параметр `-print0` в вызов утилиты `find`:
{line-numbers: false, format: Bash}
```

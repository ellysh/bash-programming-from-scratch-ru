...
Реализуем скрипт для переключения между конфигурационными файлами с использованием [символьных ссылок](https://ru.wikipedia.org/wiki/Символическая_ссылка). К сожалению, в Unix-окружении, работающем под Windows, вместо символьных ссылок будет происходить копирование файла или каталога. Однако, работая на ОС Linux вы сможете использовать ссылки. Главное их преимущество заключается в том, что при открытии ссылки вы открываете файл, на который она указывает. Это значит, что все изменения произойдут над этим файлом.

Алгоритм нашего скрипта выглядит следующим образом:

1. Удалить существующую символьную ссылку или файл по пути `~/.bashrc`

2. Проверить опцию командной строки.
...
С помощью опции, переданной через позиционный параметр `$1`, выбирается имя файла. Команда создания символьной ссылки одна остаётся неизменной. Это верный признак того, что конструкцию `case` можно заменить на ассоциативный массив. Например, следующим образом:
{line-numbers: true, format: Bash}
```
#!/bin/bash
...
Обратите внимание, что здесь мы используем двойные кавычки при подстановки элемента массива `files`. Этот элемент является одним параметром в вызове утилиты `ln`.

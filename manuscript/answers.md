...
Объединим вызовы утилит cp и bsdtar в одну команду. Утилита bsdtar вызывается только после успешного копирования файла `README`. Если между командами поставить оператор &&, получим правильную зависимость. В результате команда вызова утилит выглядит так:
{line-numbers: false, format: Bash}
```
...
Запустите эту команду. Если все шаги выполнятся успешно, в лог-файле будет следующее:
{line-numbers: true}
```
cp - OK
bsdtar - OK

...
Начнём с самого простого вывода "main1" и "main2". Переменная `var` объявлена в функции `foo`. Поэтому она доступна только в ней и в вызываемой из неё функции `bar`. Поэтому до и после вызова `foo` переменная считается необъявленной.

...
Чтобы загадать случайное число, обратимся к зарезервированной переменной `RANDOM`. При чтении она возвращает случайное значение от 0 до 32767. Нам нужно число от 1 до 100. Его можно получить из значения `RANDOM` за два шага:
...
Полная формула расчёта случайного числа от 1 до 100 выглядит так:
{line-numbers: false, format: Bash}
```
number=$((RANDOM % 100 + 1))
```

...
Чтобы вывести результат этой команды в лог-файл, воспользуемся коннектором `&&` и `echo`:
{line-numbers: false}
```
...
Как объединить получившиеся вызовы `cp` и `bsdtar`? Утилита `bsdtar` должна отработать только в случае успешного выполнения копирования. Значит, надо опять воспользоваться коннектором `&&`. В результате получим следующую команду:
{line-numbers: false}
```
...
Согласитесь, что читать получившуюся команду неудобно. Её можно разбить на несколько строк несколькими способами. Bash разрешает перенос строки сразу после коннекторов:
{line-numbers: false}
```
cp /usr/share/doc/bash/README ~ && echo "cp - OK" > ~/result.log &&
bsdtar -cjf ~/README.tar.bz2 ~/README && echo "bsdtar - OK" >> ~/result.log &&
...
Если в вашей команде нет коннекторов, вы можете воспользоваться символом обратный слэш `\`. В нашем случае для примера поставим его перед коннекторами:
{line-numbers: false}
```
cp /usr/share/doc/bash/README ~ && echo "cp - OK" > ~/result.log \
&& bsdtar -cjf ~/README.tar.bz2 ~/README && echo "bsdtar - OK" >> ~/result.log \
